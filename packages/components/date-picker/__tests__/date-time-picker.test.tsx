...
describe('Datetime Picker', () => {
  it('both picker show correct formated value (extract date-format and time-format from format property', async () => {
    ...
    const dateInput: HTMLInputElement = document.querySelector(
      '.k-date-picker__time-header > span:nth-child(1) input'
    )!
    const timeInput: HTMLInputElement = document.querySelector(
      '.k-date-picker__time-header > span:nth-child(2) input'
    )!
    timeInput.focus()
    await nextTick()
  ...
  it('both picker show correct value', async () => {
    ...
    let spinners = document.querySelectorAll(
      '.k-time-spinner ul li.is-active'
  ...
  it('click now button', async () => {
    ...
    await nextTick()
    ;(
      document.querySelector('.k-picker-panel__link-btn') as HTMLElement
    ).click()
    await nextTick()
  ...
  it('time-picker select && input time && input date', async () => {
    ...
    const input_ = document.querySelectorAll(
      '.k-date-picker__editor-wrap input'
    )[1] as HTMLElement
    input_.focus()
    await nextTick()
    const timePanel = document.querySelector('.k-time-panel')
    expect(
      timePanel!.querySelector('.k-time-spinner')!.innerHTML
    ).not.toBeNull()
    const button: HTMLElement = document.querySelector(
      '.k-time-panel .confirm'
    )!
    button.click()
    await nextTick()
  ...
  it('now button: can not choose disabled date', async () => {
    ...
    const btn: HTMLElement = document.querySelector(
      '.k-picker-panel__footer .is-text'
    )!
    btn.click()
    await nextTick()
  ...
  it('confirm button honors picked date', async () => {
    ...
    ;(
      document.querySelector(
        '.k-date-picker__header .arrow-right'
      ) as HTMLElement
    ).click()
    ;(
      document.querySelector(
        '.k-date-picker__header .d-arrow-right'
      ) as HTMLElement
    ).click()
    // click confirm button
    ;(
      document.querySelectorAll(
        '.k-picker-panel__footer .k-button'
      )[1] as HTMLElement
    ).click()
  ...
  it('defaultTime takes effect when the type is datetime', async () => {
    ...
    const someDateTd: HTMLElement = document.querySelector(
      '.k-picker-panel__content tr:nth-child(3) td:nth-child(4)'
    )!
    const timeInput: HTMLInputElement = document.querySelector(
      '.k-date-picker__time-header > span:nth-child(2) input'
    )!
  ...
  it('defaultTime only takes effect when time is not selected', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const cells = document.querySelectorAll('.available .k-date-table-cell')
  ...
  it('first click accuracy', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const dayItems = document.querySelectorAll('.k-date-table-cell__text')
...
describe('Datetimerange', () => {
  it('select daterange and default Time and input format', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const pickers = document.querySelectorAll('.k-date-range-picker__content')
    ...
    const pickerss = document.querySelectorAll(
      '.k-date-range-picker__time-header .k-date-range-picker__editors-wrap'
    )
    const left = {
      dateInput: pickerss[0].querySelector(
        '.k-date-range-picker__time-picker-wrap:nth-child(1) input'
      ),
      timeInput: pickerss[0].querySelector(
        '.k-date-range-picker__time-picker-wrap:nth-child(2) input'
      ),
    }
    const right = {
  ...
  it('select time', async () => {
    ...
    await nextTick()
    const timeInput = document.querySelectorAll(
      '.k-date-range-picker__editors-wrap input'
    )[1] as HTMLInputElement
    timeInput.blur()
    timeInput.focus()
    timeInput.blur()
    await nextTick()
    const button: HTMLElement = document.querySelector(
      '.k-date-range-picker__time-picker-wrap .k-time-panel .confirm'
    )!
    button.click()
    await nextTick()
    const btn = document.querySelectorAll(
  ...
  it('confirm honors disabledDate', async () => {
    ...
    btn.click()
    await nextTick()
    const rangePanel = document.querySelector('.k-date-range-picker')!
  ...
  it('selectableRange', async () => {
    ...
    const listLeft = document.querySelectorAll(
      '.k-date-range-picker__editors-wrap .k-time-spinner__list'
    )
    const hoursK = listLeft[0]
    const disabledHours = Array.from(
      hoursK.querySelectorAll('.is-disabled')
    ).map((node) => Number(node.textContent))
    expect(disabledHours).toStrictEqual(disabledHoursArr)
    ...
    const listRight = document.querySelectorAll(
      '.k-date-range-picker__editors-wrap.is-right .k-time-spinner__list'
    )
    const hoursK2 = listRight[0]
    const disabledHours2 = Array.from(
      hoursK2.querySelectorAll('.is-disabled')
    ).map((node) => Number(node.textContent))
  ...
  it('select same date, different time', async () => {
    ...
    leftTimeInput.focus()
    await nextTick()
    const leftList = document.querySelectorAll('.k-time-spinner__list')
    ...
    await nextTick()
    ;(
      document.querySelector('.k-time-panel__btn.confirm') as HTMLElement
    ).click()
    await nextTick()
    ...
    const rightList = document.querySelectorAll(
      '.is-right .k-time-spinner__list'
    ) as any
    // auto set left time to right time
    expect(
      rightList[0]!.querySelector('.k-time-spinner__item.is-active').innerHTML
    ).toBe(leftSelect[0])
    expect(
      rightList[1].querySelector('.k-time-spinner__item.is-active').innerHTML
    ).toBe(leftSelect[1])
    expect(
      rightList[2].querySelector('.k-time-spinner__item.is-active').innerHTML
    ).toBe(leftSelect[2])
    ...
    ;(
      document.querySelector(
        '.is-right .k-time-panel__btn.confirm'
      ) as HTMLElement
    ).click()
    await nextTick()
    ;(
  ...
  it('prop defaultTime should not confilt with prop shortcuts', async () => {
    ...
    ;(
      document.querySelector(
        '.k-picker-panel__sidebar .k-picker-panel__shortcut'
      ) as HTMLElement
    ).click()
    await nextTick()
