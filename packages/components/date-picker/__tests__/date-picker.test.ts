...
import ConfigProvider from '@kbt-ui/components/config-provider'
import { CommonPicker } from '@kbt-ui/components/time-picker'
import Input from '@kbt-ui/components/input'
import zhCn from '@kbt-ui/locale/lang/zh-cn'
import enUs from '@kbt-ui/locale/lang/en'
...
describe('DatePicker', () => {
  it('create & custom class & style', async () => {
    ...
    const outterInput = wrapper.find('.k-input')
    ...
    expect(
      document
        .querySelector('.k-picker__popper')
  ...
  it('select date', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const spans = document.querySelectorAll('.k-date-picker__header-label')
    const arrowLeftKm = document.querySelector(
      '.k-date-picker__prev-btn .arrow-left'
    ) as HTMLElement
    const arrowRightKm = document.querySelector(
      '.k-date-picker__next-btn .arrow-right'
    ) as HTMLElement
    expect(spans[0].textContent).toContain(date.year())
    expect(spans[1].textContent).toContain(date.format('MMMM'))
    const arrowLeftYeayKm = document.querySelector(
      '.k-date-picker__prev-btn .d-arrow-left'
    ) as HTMLElement
    arrowLeftYeayKm.click()
    let count = 20
    while (--count) {
      arrowLeftKm.click()
    }
  ...
  it('opens popper on click when input is focused', async () => {
    const wrapper = _mount(`<k-date-picker v-model="value" />`, () => ({
      value: new Date(2016, 9, 10, 18, 40),
    }))
    const popperK = document.querySelector('.k-picker__popper') as HTMLElement
    expect(popperK.style.display).toBe('none')
    const input = wrapper.find('input')
    input.element.focus()
    input.trigger('mousedown')
    await nextTick()
    ...
    expect(popperK.style.display).not.toBe('none')
  })
  ...
  it('shortcuts', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const shortcut = document.querySelector('.k-picker-panel__shortcut')
    expect(shortcut.textContent).toBe(text)
    expect(document.querySelector('.k-picker-panel__sidebar')).not.toBeNull()
  ...
  it('ref focus', async () => {
    ...
    await nextTick()
    await rAF()
    const popperK = document.querySelector('.k-picker__popper')
    const attr = popperK.getAttribute('aria-hidden')
  ...
  it('custom content comment', async () => {
    ...
    await nextTick()
    const el = document.querySelector('td.available .k-date-table-cell')
...
describe('DatePicker Navigation', () => {
  ...
  const initNavigationTest = async (value) => {
    ...
    getYearLabel = () =>
      document.querySelectorAll('.k-date-picker__header-label')[0].textContent
    getMonthLabel = () =>
      document.querySelectorAll('.k-date-picker__header-label')[1].textContent
  }
  ...
  it('month label with fewer dates', async () => {
    ...
    const yearLabel = document.querySelectorAll(
      '.k-date-picker__header-label'
    )[0]
    ;(yearLabel as HTMLElement).click()
    await nextTick()
    const year1999Label = document.querySelectorAll('.k-year-table td')[1]
    ;(year1999Label as HTMLElement).click()
    await nextTick()
    const juneLabel = document.querySelectorAll('.k-month-table td')[5]
    ...
    await nextTick()
    const janLabel = document.querySelectorAll('.k-month-table td')[0]
...
describe('MonthPicker', () => {
  ...
  />`,
    ...
    await nextTick()
    expect(
      (document.querySelector('.k-month-table') as HTMLElement).style.display
    ).toBe('')
    expect(document.querySelector('.k-year-table')).toBeNull()
    ;(document.querySelector('.k-month-table .cell') as HTMLElement).click()
    await nextTick()
    const vm = wrapper.vm as any
...
describe('YearPicker', () => {
  ...
  />`,
    ...
    await nextTick()
    expect(
      (document.querySelector('.k-year-table') as HTMLElement).style.display
    ).toBe('')
    expect(document.querySelector('.k-month-table')).toBeNull()
    ...
    await nextTick()
    ;(document.querySelector('.k-year-table .cell') as HTMLElement).click()
    await nextTick()
    const vm = wrapper.vm as any
  ...
  it('value-format', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const cell = document.querySelector('.k-year-table .cell') as HTMLElement
    cell.click()
    await nextTick()
    expect((wrapper.vm as any).value).toBe(
...
describe('WeekPicker', () => {
  ...
  />`,
    ...
    await nextTick()
    ;(
      document.querySelectorAll('.k-month-table .cell')[7] as HTMLElement
    ).click()
    await nextTick()
    expect(document.querySelector('.is-week-mode')).not.toBeNull()
    const numberOfHighlightRows = () =>
      document.querySelectorAll('.k-date-table__row.current').length
    ;(
      document.querySelector(
        '.k-date-table__row ~ .k-date-table__row td.available'
      ) as HTMLElement
    ).click()
    await nextTick()
    const vm = wrapper.vm as any
  ...
  ;[
    ...
    it(`emit first day of the week, ${loObj.locale.name} locale, ${loObj.name}`, async () => {
      const wrapper = mount(
        {
          components: {
            'k-date-picker': DatePicker,
            'k-config-provider': ConfigProvider,
          },
          template: `
      ...
      ;(
        document.querySelectorAll(
          '.k-date-table__row ~ .k-date-table__row td'
        )[3] as HTMLElement
      ).click()
      await nextTick()
      const vm = wrapper.vm as any
...
describe('DatePicker dates', () => {
  ...
  />`,
    ...
    expect(
      document.querySelectorAll('.k-date-table__row .selected').length
    ).toBe(2)
    td[0].click()
    await nextTick()
    expect(vm.value.length).toBe(1)
...
describe('DatePicker keyboard events', () => {
  ...
  />`,
    ...
    const input = wrapper.find('.k-input__inner')
    await input.trigger('focus')
    await input.trigger('click')
    await nextTick()

    const popperK = document.querySelectorAll('.k-picker__popper')[0]
    const attr = popperK.getAttribute('aria-hidden')
    expect(attr).toEqual('false')
    ...
    const popperK2 = document.querySelectorAll('.k-picker__popper')[0]
    const attr2 = popperK2.getAttribute('aria-hidden')
...
describe('DateRangePicker', () => {
  it('create & custom class & style', async () => {
    ...
    const outterInput = wrapper.find('.k-range-editor.k-input__wrapper')
    expect(outterInput.classes()).toContain(customClassName)
    expect(outterInput.attributes().style).toBeDefined()
    const panels = document.querySelectorAll('.k-date-range-picker__content')
  ...
  it('range, start-date and end-date', async () => {
    ...
    const table = document.querySelector('.k-date-table')
  ...
  it('unlink:true', async () => {
    ...
    await nextTick()
    const panels = document.querySelectorAll('.k-date-range-picker__content')
    const left = panels[0].querySelector('.k-date-range-picker__header')
    const right = panels[1].querySelector(
      '.is-right .k-date-range-picker__header'
    )
...
describe('MonthRange', () => {
  ...
  it('range, start-date and end-date', async () => {
    ...
    const table = document.querySelector('.k-month-table')
  ...
  describe('form item accessibility integration', () => {
    it('automatic id attachment', async () => {
      ...
      const datePickerInput = wrapper.find('.k-input__inner')
  ...
  it('prop defaultTime should not confilt with prop shortcuts', async () => {
    ...
    await nextTick()
    document
      .querySelector('.k-picker-panel__sidebar .k-picker-panel__shortcut')
      .click()
    await nextTick()
    const vm = wrapper.vm as any
    expect(vm.value).toBeDefined()
