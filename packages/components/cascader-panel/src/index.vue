...
import KCascaderMenu from './menu.vue'
...
import type { KCascaderPanelContext } from './types'
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const lazyLoad: KCascaderPanelContext['lazyLoad'] = (node, cb) => {
      const cfg = config.value
    ...
    const expandNode: KCascaderPanelContext['expandNode'] = (node, silent) => {
      const { level } = node
    ...
    const handleCheckChange: KCascaderPanelContext['handleCheckChange'] = (
      node,
      checked,
      emitClose = true
    ) => {
