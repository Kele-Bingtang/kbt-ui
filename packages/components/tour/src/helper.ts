...
import { isArray, isClient, isFunction, isString, keysOf } from '@kbt-ui/utils'
...
export const useTarget = (
  ...
  const getTargetK = () => {
    let targetK: HTMLElement | null | undefined
    if (isString(target.value)) {
      targetK = document.querySelector<HTMLElement>(target.value)
    } else if (isFunction(target.value)) {
      targetK = target.value()
    } else {
      targetK = target.value
    }
    return targetK
  ...
  const updatePosInfo = () => {
    const targetK = getTargetK()
    if (!targetK || !open.value) {
      posInfo.value = null
      return
    }
    if (!isInViewPort(targetK) && open.value) {
      targetK.scrollIntoView(scrollIntoViewOptions.value)
    }
    const { left, top, width, height } = targetK.getBoundingClientRect()
    posInfo.value = {
      left,
      top,
      width,
  ...
  const triggerTarget = computed(() => {
    const targetK = getTargetK()
    if (!mergedMask.value || !targetK || !window.DOMRect) {
      return targetK || undefined
    }
...
export const tourKey: InjectionKey<TourContext> = Symbol('KTour')
...
export const useFloating = (
  ...
  const update = async () => {
    ...
    const referenceK = unref(referenceRef)
  ...
  onMounted(() => {
    const referenceK = unref(referenceRef)
    const contentK = unref(contentRef)
    if (referenceK && contentK) {
      cleanup = autoUpdate(referenceK, contentK, update)
    }
