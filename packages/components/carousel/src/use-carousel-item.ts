...
import { debugWarn, isUndefined } from '@kbt-ui/utils'
...
export const useCarouselItem = (
  ...
  if (!carouselContext) {
    debugWarn(
      componentName,
      'usage: <k-carousel></k-carousel-item></k-carousel>'
    )
  }
  ...
  function calcTranslate(
    ...
    const rootK = carouselContext.root.value
    if (!rootK) return 0

    const distance = (isVertical ? rootK.offsetHeight : rootK.offsetWidth) || 0
