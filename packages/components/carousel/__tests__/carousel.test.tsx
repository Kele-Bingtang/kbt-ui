...
describe('Carousel', () => {
  ...
  it('create', () => {
    ...
    expect(wrapper.findAll('.k-carousel__item').length).toEqual(3)
  })
  ...
  it('auto play', async () => {
    ...
    await nextTick()
    await wait(10)
    const items = wrapper.vm.$el.querySelectorAll('.k-carousel__item')
  ...
  it('initial index', async () => {
    ...
    expect(
      wrapper.vm.$el
        .querySelectorAll('.k-carousel__item')[1]
  ...
  it('label', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.k-carousel__button span').text()).toBe('1')
  })
  ...
  describe('manual control', () => {
    it('hover', async () => {
      ...
      await nextTick()
      await wait()
      await wrapper.findAll('.k-carousel__indicator')[1].trigger('mouseenter')
      await nextTick()
      await wait()
      expect(
        wrapper.vm.$el
  ...
  it('card', async () => {
    ...
    await wait()
    expect(items[1].classList.contains('is-active')).toBeTruthy()
    await wrapper.vm.$el.querySelector('.k-carousel__arrow--left').click()
    await wait()
    expect(items[0].classList.contains('is-active')).toBeTruthy()
    await items[6].click()
    await wait()
  ...
  it('pause auto play on hover', async () => {
    ...
    await nextTick()
    await wrapper.find('.k-carousel').trigger('mouseenter')
    const items = wrapper.vm.$el.querySelectorAll('.k-carousel__item')
    await nextTick()
    await wait(60)
    expect(items[1].classList.contains('is-active')).toBeTruthy()
  ...
  it('motion blur', async () => {
    ...
    await nextTick()
    await wait(100)
    const items = wrapper.vm.$el.querySelectorAll('.k-transitioning')
  ...
  it('should guarantee order of indicators', async () => {
    ...
    data.splice(1, 0, 5)
    await nextTick()
    const indicators = wrapper.findAll('.k-carousel__button')
