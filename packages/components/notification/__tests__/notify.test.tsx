...
import { KNotification } from '..'

import type { NotificationHandle } from '../src/notification'

const selector = '.k-notification'
...
describe('Notification on command', () => {
  ...
  it('it should be able to render all types notification', () => {
    for (const type of ['success', 'warning', 'error', 'info'] as const) {
      Notification[type]({})
      expect(document.querySelector(`.k-icon-${type}`)).toBeDefined()
    }
  })
  ...
  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(KNotification._context).toBe(null)
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      KNotification.install?.(testContext as any)
      expect(KNotification._context).not.toBe(null)
      expect(KNotification._context).toBe(testContext._context)
      // clean up
      KNotification._context = null
    })
  })
})
