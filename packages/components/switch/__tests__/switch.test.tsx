...
import { debugWarn } from '@kbt-ui/utils'
...
describe('Switch.vue', () => {
  ...
  test('create', () => {
    ...
    const coreK = vm.$el.querySelector('.k-switch__core')
    expect(coreK.style.width).toEqual('100px')
    const leftLabelWrapper = wrapper.find('.k-switch__label--left span')
    expect(leftLabelWrapper.text()).toEqual('off')
  })

  test('size', () => {
    const wrapper = mount(() => <Switch size="large" />)
    expect(wrapper.find('.k-switch--large').exists()).toBe(true)
  })

  test('tabindex', () => {
    const wrapper = mount(() => <Switch tabindex="0" />)
    expect(wrapper.find('.k-switch__input').attributes().tabindex).toBe('0')
  })
  ...
  test('inline prompt', () => {
    ...
    const coreK = vm.$el.querySelector('.k-switch__core')
    expect(coreK.style.width).toEqual('100px')
    const label = wrapper.find('.k-switch__inner span')
  ...
  test('value correctly update', async () => {
    ...
    const coreWrapper = wrapper.find('.k-switch__core')
  ...
  test('sets checkbox value', async () => {
    ...
    value.value = false
    await vm.$nextTick()
    expect(inputK.checked).toEqual(false)
  })
  ...
  test('custom action icon', async () => {
    ...
    const switchWrapper = wrapper.findComponent(Switch)
    const switchVm = switchWrapper.vm
    const inputK = switchVm.$el.querySelector('input')
  ...
  describe('form item accessibility integration', () => {
    test('automatic id attachment', async () => {
      ...
      const switchInput = wrapper.find('.k-switch__input')
    ...
    test('custom switch action slots', async () => {
      ...
      const coreWrapper = wrapper.find('.k-switch__core')
      const actionWrapper = coreWrapper.find('.k-switch__action')
