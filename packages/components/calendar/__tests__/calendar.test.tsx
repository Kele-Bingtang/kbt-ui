...
describe('Calendar.vue', () => {
  it('create', async () => {
    ...
    const titleK = wrapper.find('.k-calendar__title')
    expect(/2019.*April/.test(titleK.element?.innerHTML)).toBeTruthy()
    expect(wrapper.element.querySelectorAll('thead th').length).toBe(7)
    const rows = wrapper.element.querySelectorAll('.k-calendar-table__row')
    expect(rows.length).toBe(5)
    ;(rows[4].lastElementChild as HTMLElement).click()

    await nextTick()
    expect(/2019.*May/.test(titleK.element.innerHTML)).toBeTruthy()
    const vm = wrapper.vm
    const date = vm.value
  ...
  it('range', () => {
    ...
    const titleK = wrapper.find('.k-calendar__title')
    expect(/2019.*March/.test(titleK.element.innerHTML)).toBeTruthy()
    const rows = wrapper.element.querySelectorAll('.k-calendar-table__row')
    expect(rows.length).toBe(4)
    expect(
      wrapper.element.querySelector('.k-calendar__button-group')
    ).toBeNull()
  })

  // https://github.com/kbt-ui/kbt-ui/issues/3155
  it('range when the start date will be calculated to last month', () => {
    const wrapper = mount(() => (
      <Calendar range={[new Date(2021, 1, 2), new Date(2021, 1, 28)]} />
    ))
    const titleK = wrapper.find('.k-calendar__title')
    expect(/2021.*January/.test(titleK.element.innerHTML)).toBeTruthy()
    const rows = wrapper.element.querySelectorAll('.k-calendar-table__row')
    expect(rows.length).toBe(5)
    expect(
      wrapper.element.querySelector('.k-calendar__button-group')
  ...
  it('range tow monthes when the start date will be calculated to last month', async () => {
    ...
    expect(/2021.*March/.test(titleK.element.innerHTML)).toBeTruthy()
    expect(cell?.classList.contains('is-selected')).toBeTruthy()
  })
  ...
  it('firstDayOfWeek', async () => {
    ...
    const head = wrapper.element.querySelector('.k-calendar-table thead')
    expect(head?.firstElementChild?.innerHTML).toBe('Sun')
    expect(head?.lastElementChild?.innerHTML).toBe('Sat')
    const firstRow = wrapper.element.querySelector('.k-calendar-table__row')
  ...
  it('click previous month or next month', async () => {
    ...
    await nextTick()
    const btns = wrapper.findAll('.k-button')
  ...
  it('range two years', async () => {
    ...
    const titleK = wrapper.find('.k-calendar__title')
    expect(/2021.*December/.test(titleK.element.innerHTML)).toBeTruthy()
    const dateTables = wrapper.element.querySelectorAll(
      '.k-calendar-table.is-range'
    )
    expect(dateTables.length).toBe(3)
    ...
    expect(/2022.*January/.test(titleK.element.innerHTML)).toBeTruthy()
    expect(cell?.classList.contains('is-selected')).toBeTruthy()
  })
  ...
  it('slots', async () => {
    ...
    expect(wrapper.find('.k-calendar__header').text()).toEqual(AXIOM)
