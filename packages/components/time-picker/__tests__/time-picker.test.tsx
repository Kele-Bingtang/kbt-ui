...
describe('TimePicker', () => {
  ...
  it('set format && default value && set AM/PM spinner && no $attr to panel', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const list = document.querySelectorAll('.k-time-spinner__list')
    const hoursK = list[0]
    const items = hoursK.querySelectorAll('.k-time-spinner__item')
    ...
    const times = document.querySelectorAll('.k-time-spinner__list .is-active')
    expect(times[0].textContent).toBe('06 PM')
    expect(times[1].textContent).toBe('40') // default value
    expect(times[2].textContent).toBe('00')
    const panel = document.querySelector('.k-time-panel') as any
  ...
  it('select time', async () => {
    ...
    const list = document.querySelectorAll('.k-time-spinner__list')
    const hoursK = list[0]
    const minutesK = list[1]
    const secondsK = list[2]
    const hourK = hoursK.querySelectorAll('.k-time-spinner__item')[4] as any
    const minuteK = minutesK.querySelectorAll(
      '.k-time-spinner__item'
    )[36] as any
    const secondK = secondsK.querySelectorAll(
      '.k-time-spinner__item'
    )[20] as any
    ...
    expect(hourK.classList.contains('is-active')).toBeTruthy()
    expect(minuteK.classList.contains('is-active')).toBeTruthy()
    expect(secondK.classList.contains('is-active')).toBeTruthy()
  ...
  it('click confirm / cancel button', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    ;(document.querySelector('.k-time-panel__btn.cancel') as any).click()

    expect(value.value).toBe('')
    input.trigger('blur')
    input.trigger('focus')
    await nextTick()
    ;(document.querySelector('.k-time-panel__btn.confirm') as any).click()
  ...
  it('set format', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const spinnerDom = document.querySelectorAll('.k-time-spinner__wrapper')
  ...
  it('selectableRange ', async () => {
    ...
    const minutesK = list[1]
    const secondsK = list[2]
    const disabledHours = getSpinnerTextAsArray(hoursK, '.is-disabled')
    expect(disabledHours).toEqual(disabledHoursArr)
    const hourSpinners = hoursK.querySelectorAll('.k-time-spinner__item')
    ;(hourSpinners[18] as any).click()
    await nextTick()
    const disabledMinutes = getSpinnerTextAsArray(minutesK, '.is-disabled')
    expect(disabledMinutes.every((t) => t > 30 && t < 50)).toBeTruthy()
    expect(disabledMinutes.length).toEqual(19)
    ;(hourSpinners[22] as any).click()
    await nextTick()
    const enabledMinutes = getSpinnerTextAsArray(minutesK, ':not(.is-disabled)')
    const enabledSeconds = getSpinnerTextAsArray(secondsK, ':not(.is-disabled)')
  ...
  it('can auto skip when disabled', async () => {
    ...
    const list = document.querySelectorAll('.k-time-spinner__list')
    const hoursK = list[0]
    let activeHours = getSpinnerTextAsArray(hoursK, '.is-active')[0]

    expect(activeHours).toEqual(20)
    const hoursKWrapperList = document.querySelectorAll(
      '.k-time-spinner__wrapper'
    )
    const hoursKWrapper = hoursKWrapperList[0]
    const hoursKArrowDown: Element | null =
      hoursKWrapper.querySelector('.arrow-down')
    expect(hoursKArrowDown).toBeTruthy()

    const mousedownEvt = new MouseEvent('mousedown')
    const mouseupEvt = new MouseEvent('mouseup')

    const testTime = 130
    hoursKArrowDown.dispatchEvent(mousedownEvt)
    hoursKArrowDown.dispatchEvent(mouseupEvt)
    await sleep(testTime)
    activeHours = getSpinnerTextAsArray(hoursK, '.is-active')[0]
    ...
    hoursKArrowDown.dispatchEvent(new MouseEvent('mousedown'))
    hoursKArrowDown.dispatchEvent(new MouseEvent('mouseup'))
    await sleep(testTime)
    activeHours = getSpinnerTextAsArray(hoursK, '.is-active')[0]
    expect(activeHours).toEqual(20)
  })
...
describe('TimePicker(range)', () => {
  it('create', async () => {
    ...
    expect(wrapper.find('.k-range-editor--small').exists()).toBeTruthy()
    const input = wrapper.find('input')
    input.trigger('blur')
    input.trigger('focus')
    await nextTick()
    ...
    const list = document.querySelectorAll(
      '.k-time-spinner__list .k-time-spinner__item.is-active'
    )
  ...
  it('cancel button', async () => {
    ...
    expect(document.querySelector('.k-picker-panel')).toBeNull()
    input.trigger('blur')
    input.trigger('focus')
    await nextTick()
    ;(document.querySelector('.k-time-panel__btn.confirm') as any).click()
  ...
  it('clear button', async () => {
    ...
    const findInputWrapper = () => wrapper.find('.k-date-editor')
    const findClear = () => wrapper.find('.k-range__close-icon')
  ...
  it('should be able to inherit options from parent injection', async () => {
    ...
    const value = ref(new Date(2016, 9, 10, 18, 40))
    const options = ref(KPopperOptions)
    const wrapper = mount(
      () => (
        <TimePicker
          v-model={value.value}
  ...
  it('am/pm mode avoid render redundant content', async () => {
    ...
    expect(
      list[0]
        .querySelector('.k-time-spinner__item.is-active')
  ...
  describe('form item accessibility integration', () => {
    it('automatic id attachment', async () => {
      const wrapper = mount(() => (
        <KFormItem label="Foobar" data-test-ref="item">
          <TimePicker />
        </KFormItem>
      ))
      ...
      const timePickerInput = wrapper.find('.k-input__inner')
  ...
  describe('dismiss events restore picker', () => {
    ...
    it('should be able to focus back and callout picker after clear', async () => {
      ...
      expect(document.querySelector('.k-time-panel')).toBeFalsy()
      await input.vm.$emit('input', 'a')
      await rAF()
      expect(document.querySelector('.k-time-panel')).toBeTruthy()
    })
