...
import { debugWarn, isArray } from '@kbt-ui/utils'
...
const elPopperOptions = inject('KPopperOptions', {} as Options)
...
const popperK = computed(() => unref(refPopper)?.popperRef?.contentRef)
...
onClickOutside(actualInputRef, (e: PointerEvent) => {
  const unrefedPopperK = unref(popperK)
  const inputK = unref(actualInputRef)
  if (
    (unrefedPopperK &&
      (e.target === unrefedPopperK ||
        e.composedPath().includes(unrefedPopperK))) ||
    e.target === inputK ||
    e.composedPath().includes(inputK)
  )
    return
  pickerVisible.value = false
})
