...
describe('Transfer', () => {
  ...
  it('create', () => {
    ...
    expect(wrapper.findComponent({ name: 'KTransfer' })).toBeTruthy()
  })
  ...
  it('default target list', () => {
    ...
    const KTransfer: any = wrapper.findComponent({ name: 'KTransfer' })
    expect(KTransfer.vm.sourceData.length).toBe(13)
  })
  ...
  it('filterable', async () => {
    ...
    const leftList: any = wrapper.findComponent({ name: 'KTransferPanel' })
    leftList.vm.query = '1'
  ...
  it('transfer', async () => {
    ...
    KTransfer.vm.addToLeft()
    await nextTick()
    expect(KTransfer.vm.sourceData.length).toBe(14)
    KTransfer.vm.addToRight()
    await nextTick()
    expect(KTransfer.vm.sourceData.length).toBe(12)
  })
  ...
  it('customize', () => {
    ...
    const label = wrapper.find('.k-transfer-panel__header .k-checkbox__label')
    expect(label.text().includes('表1')).toBeTruthy()
    expect(
      wrapper.find('.k-transfer-panel__list .k-checkbox__label span').text()
    ).toBe('1 - 备选项 1')
  ...
  describe('target order', () => {
    it('original(default)', async () => {
      ...
      const targetItems = wrapper.findAll(
        '.k-transfer__buttons + .k-transfer-panel .k-transfer-panel__body .k-checkbox__label span'
      )
