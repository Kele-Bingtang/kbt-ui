...
import { triggerNativeCompositeClick } from '@kbt-ui/test-utils/composite-click'
import { QuestionFilled as QuestionFilledIcon } from '@element-plus/icons-vue'
import MessageBox from '../src/messageBox'
import { KMessageBox } from '..'

const selector = '.k-overlay'
...
describe('MessageBox', () => {
  ...
  test('create and close', async () => {
    ...
    await rAF()
    expect(
      msgbox.querySelector('.k-message-box__title span').textContent
    ).toEqual('消息')
    expect(
      msgbox.querySelector('.k-message-box__message').querySelector('p')
        .textContent
    ).toEqual('这是一段内容')
    /** custom inline style */
    expect(
      (msgbox.querySelector('.k-message-box') as HTMLElement).style.width
    ).toEqual('100px')
    MessageBox.close()
    await rAF()
  ...
  test('custom icon', async () => {
    ...
    await rAF()
    const icon = document.querySelector('.k-message-box__status')

    expect(icon.classList.contains('k-icon')).toBe(true)
  ...
  test('html string', async () => {
    ...
    await rAF()
    const message = document.querySelector('.k-message-box__message strong')
  ...
  test('alert', async () => {
    ...
    expect(msgbox.querySelector('.k-icon-warning')).toBeDefined()
  })
  ...
  test('confirm', async () => {
    ...
    const btn = document
      ...
      .querySelector('.k-button--primary') as HTMLButtonElement
    btn.click()
    await rAF()
    const msgbox: HTMLElement = document.querySelector(selector)
  ...
  test('autofocus', async () => {
    ...
    await rAF()
    const btnKm = document.querySelector(
      '.k-message-box__btns .k-button--primary'
    )
    const haveFocus = btnKm.isSameNode(document.activeElement)
  ...
  test('prompt', async () => {
    ...
    await rAF()
    const inputKm = document
      .querySelector(selector)
      .querySelector('.k-message-box__input')
    const haveFocus = inputKm
      .querySelector('input')
      .isSameNode(document.activeElement)
    expect(inputKm).toBeDefined()
  ...
  test('prompt: focus on textarea', async () => {
    ...
    await rAF()
    const textareaKm = document
      .querySelector(selector)
      .querySelector('textarea')
    const haveFocus = textareaKm.isSameNode(document.activeElement)
    expect(haveFocus).toBe(true)
  })
  ...
  describe('promise', () => {
    ...
    test('reject', async () => {
      ...
      await rAF()
      const btn = document.querySelector('.k-message-box__btns .k-button')
  ...
  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(KMessageBox._context).toBe(null)
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      KMessageBox.install?.(testContext as any)
      expect(KMessageBox._context).not.toBe(null)
      expect(KMessageBox._context).toBe(testContext._context)
      // clean up
      KMessageBox._context = null
    })
  })
