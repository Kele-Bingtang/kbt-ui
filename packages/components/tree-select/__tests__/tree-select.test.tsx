...
import type KSelect from '@kbt-ui/components/select'
import type KTree from '@kbt-ui/components/tree'
...
const createComponent = ({
  ...
  return {
    ...
    select: wrapper.findComponent({ name: 'KSelect' }) as VueWrapper<
      InstanceType<typeof KSelect>
    >,
    tree: wrapper.findComponent({ name: 'KTree' }) as VueWrapper<
      InstanceType<typeof KTree>
    >,
  }
}
...
describe('TreeSelect.vue', () => {
  test('render test', async () => {
    ...
    expect(wrapper.find('.k-tree')).toBeTruthy()
    expect(wrapper.find('.k-select')).toBeTruthy()

    expect(tree.findAll('.k-tree > .k-tree-node').length).toBe(1)
    expect(tree.findAll('.k-tree .k-tree-node').length).toBe(3)
    expect(tree.findAll('.k-tree .k-select-dropdown__item').length).toBe(3)
  ...
  test('modelValue', async () => {
    ...
    await tree
      .findAll('.k-select-dropdown__item')
      .slice(-1)[0]
      .trigger('click')
    await nextTick()
    ...
    await tree.find('.k-tree-node__content').trigger('click')
    await nextTick()
    expect(select.vm.modelValue).toBe(1)
    expect(wrapperRef.getCheckedKeys()).toEqual([1])

    await tree.findAll('.k-checkbox__original')[1].trigger('click')
    await nextTick()
    expect(select.vm.modelValue).toBe(11)
    expect(wrapperRef.getCheckedKeys()).toEqual([11])
  })
  ...
  test('disabled', async () => {
    ...
    await nextTick()
    await tree.find('.k-tree-node').trigger('click')
    await tree.find('.k-tree-node .k-checkbox.is-disabled').trigger('click')
    await tree
      .find('.k-tree-node .k-select-dropdown__item.is-disabled')
      .trigger('click')
    await nextTick()
  ...
  test('multiple', async () => {
    ...
    await tree.findAll('.k-checkbox')[1].trigger('click')
    await nextTick()
  ...
  test('filter', async () => {
    ...
    tree.vm.filter('一级 1')
    await nextTick()
    expect(tree.findAll('.k-tree-node:not(.is-hidden)').length).toBe(1)
  })
  ...
  test('props', async () => {
    ...
    await nextTick()
    expect(tree.find('.k-select-dropdown__item').text()).toBe('1')
  ...
  test('slots', async () => {
    ...
    await nextTick()
    expect(tree.find('.k-select-dropdown__item').text()).toBe('123一级 1')
    expect(select.find('.k-select__prefix').text()).toBe('prefix')
  })
  ...
  test('lazy', async () => {
    ...
    await tree.find('.k-tree-node').trigger('click')
    await nextTick()
    expect(tree.find('.k-tree-node .k-tree-node').text()).toBe('2')
  })
  ...
  test('check-strictly showCheckbox click node', async () => {
    ...
    const wrapperRef = await getWrapperRef()
    await tree.findAll('.k-tree-node__content')[0].trigger('click')
    await nextTick()
    expect(select.vm.modelValue).toEqual([])
    expect(wrapperRef.getCheckedKeys()).toEqual([])

    await tree
      .findAll('.k-tree-node__content .k-checkbox')[0]
      .trigger('click')
    await nextTick()
    expect(select.vm.modelValue).toEqual([1])
    expect(wrapperRef.getCheckedKeys()).toEqual([1])
  ...
  test('only show checkbox', async () => {
    ...
    await tree
      .find('.k-tree-node__content .k-checkbox__original')
      .trigger('click')
    await nextTick()
  ...
  test('show checkbox and check on click node', async () => {
    ...
    // check child node when folder node checked,
    // value.value will be 111
    await tree.findAll('.k-tree-node__content').slice(-1)[0].trigger('click')
    await nextTick()
    expect(select.vm.modelValue).equal(111)
  ...
  test('expand-on-click-node', async () => {
    ...
    await tree.findAll('.k-tree-node__content')[0].trigger('click')
    expect(
      tree.findAll('.k-tree-node__children')[0].attributes('style')
  ...
  describe('filter method and remote method', () => {
    ...
    const testMethod = async (props: any) => {
      ...
      const testKeywords = async (keywords: string) => {
        ...
        expect(
          tree.findAll('.k-select-dropdown__item').map((item) => item.text())
        ).toEqual([keywords, `${keywords}-child`])

        const treeNode = tree.find('.k-tree-node')
  ...
  test('check/check-change events can accept correct params', async () => {
    ...
    await nextTick()
    await tree.find('.k-checkbox__original').trigger('click')
    await nextTick()
  ...
  test('checking node will not reset checked cache node', async () => {
    ...
    const node1 = tree.find('.k-tree-node__content')
    const node1Checkbox = node1.find('.k-checkbox__original')
  ...
  test('cached checked node can be canceled', async () => {
    ...
    const node2 = tree.findAll('.k-tree-node__content')[1]
    expect(node2.text()).toBe('2-label')

    const node2Checkbox = node2.find('.k-checkbox')
  ...
  test('no checkbox and check on click node', async () => {
    ...
    const nodes = tree.findAll('.k-tree-node__content')
  ...
  test('always focus when using filters', async () => {
    ...
    // checkbox click
    await tree
      .findAll('.k-tree-node__content .k-checkbox')[1]
      .trigger('click')
