...
import { flattedChildren } from '@kbt-ui/utils'
import { useNamespace } from '@kbt-ui/hooks'
import { useFormSize } from '@kbt-ui/components/form'
import KDescriptionsRow from './descriptions-row.vue'
...
const getRows = () => {
  ...
  const children = flattedChildren(slots.default()).filter(
    (node): node is DescriptionItemVNode =>
      (node as any)?.type?.name === 'KDescriptionsItem'
  )
