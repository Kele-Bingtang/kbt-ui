...
import { makeMountFunc } from '@kbt-ui/test-utils/make-mount'
...
describe('Select', () => {
  ...
  it('custom dropdown class', async () => {
    ...
    await nextTick()
    expect([...document.querySelector('.k-popper').classList]).toContain(
      'custom-dropdown'
    )
  })
  ...
  describe('multiple', () => {
    it('multiple select', async () => {
      ...
      const tagIcon = wrapper.find('.k-tag__close')
  ...
  describe('collapseTags', () => {
    it('use collapseTags', async () => {
      ...
      await nextTick()
      expect(vm.value.length).toBe(3)
      const tags = wrapper.findAll('.k-tag').filter((item) => {
    ...
    it('use collapseTagsTooltip', async () => {
      ...
      await nextTick()
      expect(vm.value.length).toBe(3)
      expect(wrapper.findAll('.k-tag')[1].element.textContent.trim()).toBe(
        '+ 2'
      )
    })
  ...
  describe('manually set modelValue', () => {
    ...
    it('set modelValue in multiple select', async () => {
      ...
      vm.value = ['option_1']
      await nextTick()
      expect(wrapper.find('.k-select__tags-text').text()).toBe('a0')
      placeholder = wrapper.find(`.${PLACEHOLDER_CLASS_NAME}`)
      expect(placeholder.exists()).toBeFalsy()

      vm.value = []
      await nextTick()
      expect(wrapper.find('.k-select__tags-text').exists()).toBeFalsy()
      placeholder = wrapper.find(`.${PLACEHOLDER_CLASS_NAME}`)
      expect(placeholder.exists()).toBeTruthy()
    })
      ...
      slots: {
        default: `
          <div class="custom-renderer">
            ...
            <span style="color: var(--k-text-color-secondary); font-size: 13px">
              {{ item.value }}
            </span>
          </div>
        `,
  ...
  it('multiple select when content overflow', async () => {
    ...
    const select = wrapper.findComponent(Select)
    const selectVm = select.vm as any
    const selectDom = wrapper.find('.k-select__wrapper').element
    const selectRect = {
      height: 40,
      width: 221,
      x: 44,
    ...
    options[2].click()
    await nextTick()
    const tagWrappers = wrapper.findAll('.k-select__tags-text')
  ...
  describe('scrollbarAlwaysOn flag control the scrollbar whether always displayed', () => {
    it('The default scrollbar is not always displayed', async () => {
      ...
      await wrapper.trigger('click')
      expect((select.vm as any).expanded).toBeTruthy()
      const box = document.querySelector<HTMLElement>('.k-vl__wrapper')
