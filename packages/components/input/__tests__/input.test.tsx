...
import { KFormItem as FormItem } from '@kbt-ui/components/form'
...
describe('Input.vue', () => {
  ...
  test('create', async () => {
    ...
    const inputKm = wrapper.find('input')
    const nativeInput = inputKm.element

    await inputKm.trigger('focus')

    expect(inputKm.exists()).toBe(true)
    ...
    input.value = 'text'
    await nextTick()
    expect(inputKm.element.value).toMatchInlineSnapshot(`"text"`)
  })

  test('default to empty', () => {
    const wrapper = mount(() => <Input />)
    const inputKm = wrapper.find('input')
    expect(inputKm.element.value).toBe('')
  })

  test('disabled', () => {
    const wrapper = mount(() => <Input disabled />)
    const inputKm = wrapper.find('input')
    expect(inputKm.element.disabled).not.toBeNull()
  })

  describe('test emoji', () => {
    test('k-input should minimize value between emoji length and maxLength', async () => {
      ...
      const elCount = wrapper.find('.k-input__count-inner')
    ...
    test('textarea should minimize value between emoji length and maxLength', async () => {
      ...
      const vm = wrapper.vm
      const inputKm = wrapper.find('textarea')
      const nativeInput = inputKm.element
      expect(nativeInput.value).toMatchInlineSnapshot(`"啊好😄"`)

      const elCount = wrapper.find('.k-input__count')
      expect(elCount.exists()).toBe(true)
      expect(elCount.text()).toMatchInlineSnapshot(`"4 / 4"`)
  ...
  test('suffixIcon', () => {
    ...
    const icon = wrapper.find('.k-input__icon')
  ...
  test('size', () => {
    ...
    expect(wrapper.classes('k-input--large')).toBe(true)
  })

  test('type', () => {
    const wrapper = mount(() => <Input type="textarea" />)
    expect(wrapper.classes('k-textarea')).toBe(true)
  })
  ...
  test('limit input and show word count', async () => {
    ...
    const inputKm1 = wrapper.vm.$el.querySelector('.test-text')
    const inputKm2 = wrapper.vm.$el.querySelector('.test-textarea')
    const inputKm3 = wrapper.vm.$el.querySelector('.test-password')
    const inputKm4 = wrapper.vm.$el.querySelector('.test-initial-exceed')

    expect(inputKm1.querySelectorAll('.k-input__count').length).toEqual(0)
    expect(inputKm2.querySelectorAll('.k-input__count').length).toEqual(1)
    expect(inputKm3.querySelectorAll('.k-input__count').length).toEqual(0)
    expect(Array.from(inputKm4.classList)).toMatchInlineSnapshot(`
      [
        "k-input",
        "is-exceed",
        "test-initial-exceed",
    ...
    show.value = true
    await nextTick()
    expect(inputKm1.querySelectorAll('.k-input__count').length).toEqual(1)
  ...
  describe('Input Events', () => {
    ...
    test('event:clear', async () => {
      ...
      await input.trigger('focus')
      await nextTick()
      vm.$el.querySelector('.k-input__clear').click()
      await nextTick()
  ...
  test('show-password icon', async () => {
    ...
    const icon = wrapper.find('.k-input__icon.k-input__password')
