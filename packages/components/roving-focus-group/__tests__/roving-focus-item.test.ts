...
import { composeRefs } from '@kbt-ui/utils'
...
describe('<KRovingFocusItem />', () => {
  ...
  const createComponent = (props = {}) =>
    mount(
      {
        template: `<div>
        <k-roving-focus-item v-bind="$attrs">
          <focus-item />
        </k-roving-focus-item>
        <k-roving-focus-item v-bind="$attrs">
          <focus-item />
  ...
  const findItems = () => wrapper.findAllComponents(KRovingFocusItem)
