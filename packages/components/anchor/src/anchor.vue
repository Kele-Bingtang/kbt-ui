...
const containerK = ref<HTMLElement | Window>()
...
const scrollToAnchor = (href: string) => {
  if (!containerK.value) return
  const target = getElement(href)
  if (!target) return
  if (clearAnimate) clearAnimate()
  isScrolling = true
  const scrollKe = getScrollElement(target, containerK.value)
  const distance = getOffsetTopDistance(target, scrollKe)
  const max = scrollKe.scrollHeight - scrollKe.clientHeight
...
const handleScroll = throttleByRaf(() => {
  if (containerK.value) {
    currentScrollTop = getScrollTop(containerK.value)
  }
...
const getCurrentHref = () => {
  if (!containerK.value) return
  const scrollTop = getScrollTop(containerK.value)
...
const getContainer = () => {
  ...
  if (!el || isWindow(el)) {
    containerK.value = window
  } else {
    containerK.value = el
  }
}

useEventListener(containerK, 'scroll', handleScroll)

const markerStyle = computed(() => {
  if (!anchorRef.value || !markerRef.value || !currentAnchor.value) return {}
  const currentLinkK = links[currentAnchor.value]
  if (!currentLinkK) return {}
  const anchorRect = anchorRef.value.getBoundingClientRect()
  const markerRect = markerRef.value.getBoundingClientRect()
  const linkRect = currentLinkK.getBoundingClientRect()
