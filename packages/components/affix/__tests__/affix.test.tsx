...
describe('Affix.vue', () => {
  test('render test', async () => {
    ...
    const mockAffixRect = vi
      .spyOn(wrapper.find('.k-affix').element, 'getBoundingClientRect')
      .mockReturnValue({
        height: 40,
        width: 1000,
        top: -100,
    ...
    expect(wrapper.find('.k-affix--fixed').exists()).toBe(false)
    await makeScroll(document.documentElement, 'scrollTop', 200)
    expect(wrapper.find('.k-affix--fixed').exists()).toBe(true)
  ...
  test('should render offset props', async () => {
    ...
    await makeScroll(document.documentElement, 'scrollTop', 200)
    expect(wrapper.find('.k-affix--fixed').exists()).toBe(true)
    expect(wrapper.find('.k-affix--fixed').attributes('style')).toContain(
      'top: 30px;'
    )
    mockAffixRect.mockRestore()
    mockDocumentRect.mockRestore()
