...
describe('menu', () => {
  ...
  test('create', async () => {
    const wrapper = _mount(
      `<k-menu>
        <k-menu-item index="1" ref="item1">处理中心</k-menu-item>
        <k-menu-item index="2" ref="item2">订单管理</k-menu-item>
      </k-menu>`
    )
  ...
  test('background-color', async () => {
    ...
    expect(
      window.getComputedStyle(instance)._values['--k-menu-bg-color']
  ...
  test('menu-item click', async () => {
    const wrapper = _mount(
      `<k-menu>
        <k-menu-item @click="onMenuItemClick" index="1" ref="item1">处理中心</k-menu-item>
        <k-menu-item index="2" ref="item2">订单管理</k-menu-item>
      </k-menu>`,
      {
        data() {
  ...
  test('menu-item disabled', async () => {
    const wrapper = _mount(
      `<k-menu default-active="2">
        <k-menu-item index="1" ref="item1" disabled>处理中心</k-menu-item>
        <k-menu-item index="2" ref="item2">订单管理</k-menu-item>
      </k-menu>`
    )
    const item1 = await wrapper.findComponent({ ref: 'item1' })
  ...
  test('open method', async () => {
    const wrapper = _mount(
      `<div>
          ...
          >
            <k-sub-menu index="1" ref="subMenu">
              <template #title>
                <span>导航一</span>
              </template>
              <k-menu-item-group>
                <template #title>分组一</template>
                <k-menu-item index="1-1">选项1</k-menu-item>
                <k-menu-item index="1-2">选项2</k-menu-item>
              </k-menu-item-group>
              <k-menu-item-group title="分组2">
                <k-menu-item index="1-3">选项3</k-menu-item>
              </k-menu-item-group>
              <k-sub-menu index="1-4">
                <template #title>选项4</template>
                <k-menu-item index="1-4-1">选项1</k-menu-item>
              </k-sub-menu>
            </k-sub-menu>
            <k-menu-item index="2">
  ...
  test('hover-bg-color', async () => {
    const wrapper = _mount(
      `<k-menu ref="menu" default-active="2"
...
describe('default active', () => {
  ...
  test('dynamic active', async () => {
    const wrapper = _mount(
      `<k-menu :default-active="active">
        <k-menu-item index="1" ref="item1">active watch处理中心</k-menu-item>
        <k-menu-item index="2" ref="item2">active watch订单管理</k-menu-item>
      </k-menu>`,
      {
        data() {
          return {
  ...
  test('vertical submenu item active', async () => {
    const wrapper = _mount(
      `<div>
        <k-menu default-active="2-2" mode="vertical">
          <k-menu-item index="1" ref="item1">处理中心</k-menu-item>
          <k-sub-menu index="2" ref="submenu">
            <template slot="title">我的工作台</template>
            <k-menu-item index="2-1">选项1</k-menu-item>
            <k-menu-item index="2-2" ref="submenuItem2">选项2</k-menu-item>
            <k-menu-item index="2-3">选项3</k-menu-item>
          </k-sub-menu>
          <k-menu-item index="3">订单管理</k-menu-item>
        </k-menu>
      </div>`
    )
...
describe('submenu', () => {
  test('toggle', async () => {
    ...
    const submenu = await wrapper.findComponent({ ref: 'submenu' })
    const submenuItem2 = await wrapper.findComponent({ ref: 'submenuItem2' })
    submenu.vm.$el.querySelector('.k-sub-menu__title').click()
    await nextTick()
    expect(submenu.classes()).toContain('is-opened')
  ...
  test('default opened', async () => {
    const wrapper = _mount(
      `<k-menu :default-openeds="defaultOpeneds">
        <k-menu-item index="1">default opened处理中心</k-menu-item>
        <k-sub-menu index="2" ref="submenu1">
          <template slot="title">default opened我的工作台</template>
          <k-menu-item index="2-1">选项1</k-menu-item>
          <k-menu-item index="2-2" ref="submenu1Item2">选项2</k-menu-item>
          <k-menu-item index="2-3">选项3</k-menu-item>
        </k-sub-menu>
        <k-sub-menu index="3" ref="submenu2">
          <template slot="title">default opened订单管理</template>
          <k-menu-item index="3-1">选项1</k-menu-item>
          <k-menu-item index="3-2" ref="submenu2Item2">选项2</k-menu-item>
          <k-menu-item index="3-3">选项3</k-menu-item>
        </k-sub-menu>
      </k-menu>`,
      {
        data() {
  ...
  test('disabled', async () => {
    const wrapper = _mount(
      `<k-menu>
        <k-menu-item index="1" ref="item1">处理中心</k-menu-item>
        <k-sub-menu index="2" ref="submenu" disabled>
          <template slot="title">我的工作台</template>
          <k-menu-item index="2-1">选项1</k-menu-item>
          <k-menu-item index="2-2" ref="submenuItem2">选项2</k-menu-item>
          <k-menu-item index="2-3">选项3</k-menu-item>
...
describe('other', () => {
  test('disabled', async () => {
    const wrapper = _mount(
      `<k-menu unique-opened default-active="2-2">
        <k-menu-item index="1">处理中心</k-menu-item>
        <k-sub-menu index="2" ref="submenu1">
          <template slot="title">我的工作台</template>
          <k-menu-item index="2-1">选项1</k-menu-item>
          <k-menu-item index="2-2" ref="submenu1Item2">选项2</k-menu-item>
    ...
    const submenu2 = await wrapper.findComponent({ ref: 'submenu2' })
    submenu2.vm.$el.querySelector('.k-sub-menu__title').click()
    await nextTick()
    const submenu1 = await wrapper.findComponent({ ref: 'submenu1' })
  ...
  test('horizontal mode', async () => {
    const onOpen = vi.fn()
    const wrapper = _mount(
      `<k-menu mode="horizontal" @open="onOpen">
        <k-menu-item index="1">处理中心</k-menu-item>
        <k-sub-menu index="2" ref="submenu">
          <template slot="title">我的工作台</template>
          <k-menu-item index="2-1">选项1</k-menu-item>
    ...
    expect(wrapper.classes()).toContain('k-menu--horizontal')
  ...
  test('menu group', async () => {
    const wrapper = _mount(
      `<k-menu mode="vertical" default-active="1">
        <k-menu-item-group title="分组一" ref="group1">
          <k-menu-item index="1">导航一</k-menu-item>
          <k-menu-item index="2">导航二</k-menu-item>
        </k-menu-item-group>
        <k-sub-menu index="5">
          <template slot="title">导航五</template>
          <k-menu-item-group title="分组二">
            <k-menu-item index="5-1">选项1</k-menu-item>
            <k-menu-item index="5-2">选项2</k-menu-item>
          </k-menu-item-group>
        </k-sub-menu>
      </k-menu>`
    )
    ...
    expect(
      group1.vm.$el.querySelector('.k-menu-item-group__title').innerHTML
    ).toEqual('分组一')
  })
  test('dynamic menus, issue 9092', async () => {
    ...
    await nextTick()
    expect(
      instance.$el.querySelector('.k-menu-item.is-active').innerHTML
    ).toEqual('new')
  })
})
