...
import defineGetter from '@kbt-ui/test-utils/define-getter'
import sleep from '@kbt-ui/test-utils/sleep'
    ...
    Object.assign(
      {
        ...
        template: `
      <k-tree ref="tree" :data="data" ${props}></k-tree>
    `,
        data() {
          return {
            currentNode: null,
...
describe('Tree.vue', () => {
  test('create', async () => {
    ...
    expect(wrapper.find('.k-tree').exists()).toBeTruthy()
    expect(wrapper.findAll('.k-tree > .k-tree-node').length).toEqual(3)
    expect(wrapper.findAll('.k-tree .k-tree-node').length).toEqual(
      ALL_NODE_COUNT
    )
  ...
  test('click node', async () => {
    ...
    const firstNodeContentWrapper = wrapper.find('.k-tree-node__content')
    const firstNodeWrapper = wrapper.find('.k-tree-node')
  ...
  test('emptyText', async () => {
    ...
    vm.data = []
    await nextTick()
    expect(wrapper.findAll('.k-tree__empty-block').length).toEqual(1)
  })
  ...
  test('current-node-key', async () => {
    ...
    const currentNodeLabelWrapper = wrapper.find(
      '.is-current .k-tree-node__label'
    )

    expect(currentNodeLabelWrapper.text()).toEqual('二级 1-1')
    expect(wrapper.find('.k-tree--highlight-current').exists()).toBe(true)
  })

  test('defaultExpandAll', async () => {
    const { wrapper } = getTreeVm(`:props="defaultProps" default-expand-all`)
    const expanedNodeWrappers = wrapper.findAll('.k-tree-node.is-expanded')
  ...
  test('defaultExpandedKeys set', async () => {
    ...
    await nextTick()
    let expanedNodeWrappers = wrapper.findAll('.k-tree-node.is-expanded')
    expect(expanedNodeWrappers.length).toEqual(2)
    ...
    await nextTick()
    await nextTick()
    expanedNodeWrappers = wrapper.findAll('.k-tree-node.is-expanded')
  ...
  test('filter-node-method', async () => {
    ...
    await nextTick()
    expect(treeWrapper.findAll('.k-tree-node.is-hidden').length).toEqual(3)
  })
  test('lazy load with filter expand loaded node', async () => {
    ...
    let nodeWrappers = wrapper.findAll('.k-tree-node__content')

    expect(nodeWrappers.length).toEqual(1)
    nodeWrappers[0].trigger('click')
    await sleep()
    nodeWrappers = wrapper.findAll('.k-tree-node__content')
  ...
  test('autoExpandParent = true', async () => {
    ...
    expect(wrapper.findAll('.k-tree-node.is-expanded').length).toEqual(3)
  })
  ...
  test('autoExpandParent = false', async () => {
    ...
    expect(wrapper.findAll('.k-tree-node.is-expanded').length).toEqual(0)

    const firstNodeContentWrapper = wrapper.find('.k-tree-node__content')
    await firstNodeContentWrapper.trigger('click')
    await nextTick()

    expect(wrapper.findAll('.k-tree-node.is-expanded').length).toEqual(2)
  })
  ...
  test('defaultCheckedKeys & check-strictly = false', async () => {
    ...
    expect(wrapper.findAll('.k-checkbox .is-checked').length).toEqual(3)
  })
  ...
  test('defaultCheckedKeys & check-strictly', async () => {
    ...
    expect(wrapper.findAll('.k-checkbox .is-checked').length).toEqual(1)
  })
  ...
  test('show checkbox', async () => {
    ...
    const secondNodeCheckboxWrapper =
      secondNodeContentWrapper.find('.k-checkbox')
    ...
    const secondTreeNodeWrapper = treeWrapper.findAll('.k-tree-node')[1]
    const secondNodefirstLeafCheckboxWrapper = secondTreeNodeWrapper.find(
      '.k-tree-node__children .k-tree-node__content .k-checkbox'
    )
  ...
  test('check', async () => {
    ...
    const secondNodeContentWrapper = wrapper.findAll('.k-tree-node__content')[1]
    const secondNodeCheckboxWrapper =
      secondNodeContentWrapper.find('.k-checkbox')
    expect(secondNodeCheckboxWrapper.exists()).toBe(true)
  ...
  test('setCheckedNodes', async () => {
    ...
    const treeVm = treeWrapper.vm as InstanceType<typeof Tree>
    const secondNodeContentWrapper = wrapper.findAll('.k-tree-node__content')[1]
    const secondNodeCheckWrapper = secondNodeContentWrapper.find('.k-checkbox')
  ...
  test('setCheckedNodes with disabled node', async () => {
    ...
    const wrapper = mount(
      Object.assign({
        ...
        template: `
        <k-tree ref="tree" :data="data" :props="defaultProps" default-expand-all show-checkbox node-key="id"></k-tree>
      `,
        data() {
          return {
            data: nodes,
  ...
  test('set disabled checkbox', async () => {
    ...
    const nodeWrapper = wrapper.findAll('.k-tree-node__content')[2]
    const checkboxWrapper = nodeWrapper.find('.k-checkbox input')
  ...
  test('check strictly', async () => {
    ...
    const treeWrapper = wrapper.findComponent(Tree)
    const secondNodeContentWrapper = wrapper.findAll('.k-tree-node__content')[3]
    const secondNodeCheckboxWrapper =
      secondNodeContentWrapper.find('.k-checkbox')
    await secondNodeCheckboxWrapper.trigger('click')
    expect(
    ...
    const secondTreeNodeWrapper = treeWrapper.findAll('.k-tree-node')[3]
    const secondNodefirstLeafCheckboxWrapper = secondTreeNodeWrapper.find(
      '.k-tree-node__children .k-tree-node__content .k-checkbox'
    )
    await secondNodefirstLeafCheckboxWrapper.trigger('click')
        ...
        methods: {
          renderContent(h, node) {
            return h('div', { class: 'custom-content' }, [
              h('button', { class: 'k-button' }, [node.node.label]),
            ])
          },
        },
      }
    )
    const firstNodeWrapper = wrapper.find('.k-tree-node__content')
  ...
  test('custom-node-class', async () => {
    ...
    const currentNodeLabelWrapper = wrapper.find('.is-test .k-tree-node__label')
  ...
  test('lazy defaultChecked', async () => {
    ...
    const treeWrapper = wrapper.findComponent(Tree)
    const tree = treeWrapper.vm as InstanceType<typeof Tree>
    const firstNodeWrapper = treeWrapper.find('.k-tree-node__content')
    expect(firstNodeWrapper.find('.is-indeterminate').exists()).toEqual(false)

    tree.store.setCheckedKeys([3])
    vi.useFakeTimers()
    await firstNodeWrapper.find('.k-tree-node__expand-icon').trigger('click')
    vi.runAllTimers()
    vi.useRealTimers()
    await nextTick()

    expect(firstNodeWrapper.find('.is-indeterminate').exists()).toEqual(true)
    const childWrapper = treeWrapper.findAll('.k-tree-node__content')[1]
  ...
  test('lazy without expandOnChecked', async () => {
    ...
    const nodeWrappers = treeWrapper.findAll('.k-tree-node__content')
  ...
  test('accordion', async () => {
    ...
    const secondNodeContentWrapper = wrapper.find(
      '.k-tree-node:nth-child(2) .k-tree-node__content'
    )
    await firstNodeContentWrapper.trigger('click')

    expect(wrapper.find('.k-tree-node').classes('is-expanded')).toBe(true)
    await secondNodeContentWrapper.trigger('click')
    expect(wrapper.find('.k-tree-node').classes('is-expanded')).toBe(false)
  })
  ...
  test('handleNodeOpen & handleNodeClose', async () => {
    ...
    expect(firstNodeWrapper.find('.k-tree-node__children').exists()).toBe(false)
  ...
  test('updateKeyChildren', async () => {
    ...
    const nodeContentWrapper = wrapper.findAll('.k-tree-node__content')[1]
    const nodeLabelWrapper = nodeContentWrapper.find('.k-tree-node__label')
  ...
  test('update multi tree data', async () => {
    const { wrapper, vm } = getTreeVm(``, {
      template: `
        <div>
          <k-tree ref="tree1" :data="data" node-key="id" :props="defaultProps"></k-tree>
          <k-tree ref="tree2" :data="data" node-key="id" :props="defaultProps"></k-tree>
        </div>
      `,
    })
  ...
  test('navigate with defaultExpandAll', () => {
    const { wrapper } = getTreeVm(``, {
      template: `
        <div>
          <k-tree default-expand-all ref="tree1" :data="data" node-key="id" :props="defaultProps"></k-tree>
        </div>
      `,
    })
    const tree = wrapper.findComponent({ name: 'KTree' })
    expect(
      Object.values(
  ...
  test('custom children and label', async () => {
    const wrapper = mount({
      template: `
        <div>
          <k-tree ref="tree1" :data="data" node-key="id" :props="defaultProps"></k-tree>
          <k-button @click="addNewNode">add</k-button>
        </div>
      `,
      components: {
        'k-tree': Tree,
    ...
    const firstNode = wrapper.find('.k-tree-node')
  ...
  test('customize some node contents', async () => {
    ...
    const nodeContentWrapper1 = wrapper.findAll('.k-tree-node__content')[0]
    const nodeContentWrapper2 = wrapper.findAll('.k-tree-node__content')[1]

    const nodeLabelWrapper1 = nodeContentWrapper1.find('div')
    const nodeLabelWrapper2 = nodeContentWrapper2.find(
      'span.k-tree-node__label'
    )
