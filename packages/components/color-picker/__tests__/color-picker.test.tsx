...
describe('Color-picker', () => {
  it('Mount Color-picker', () => {
    ...
    expect(wrapper.html()).toContain('k-color-picker')
    wrapper.unmount()
  })
  it('should show alpha slider when show-alpha=true', async () => {
    ...
    await wrapper.find('.k-color-picker__trigger').trigger('click')
    const alphaSlider = document.querySelector('.k-color-alpha-slider')
  ...
  it('should pick a color when confirm button click', async () => {
    ...
    await wrapper.find('.k-color-picker__trigger').trigger('click')
    document.querySelector<HTMLElement>('.k-color-dropdown__btn')?.click()
    await nextTick()
  ...
  it('should show color picker when click trigger', async () => {
    ...
    await wrapper.find('.k-color-picker__trigger').trigger('click')
    const dropdown = document.querySelector('.k-color-dropdown')
  ...
  it('should change color to the selected color', async () => {
    ...
    expect(
      predefineDom.querySelectorAll('.k-color-predefine__color-selector')
        .length === 9
    ).toBeTruthy()
    predefineDom
      .querySelector<HTMLElement>(
        '.k-color-predefine__color-selector:nth-child(4)'
      )
      ?.click()
    await nextTick()
    ...
    predefineDom
      .querySelector<HTMLElement>(
        '.k-color-predefine__color-selector:nth-child(3)'
      )
      ?.click()
    await nextTick()
  ...
  it('should change selected state of predefined color', async () => {
    ...
    expect(
      predefineWrapper
        .find('.k-color-predefine__color-selector:nth-child(4)')
        .classes()
    ).toContain('selected')
    const hueSlideWrapper = colorPickerWrapper.findComponent({ ref: 'hue' })
    const hueSlideDom = hueSlideWrapper.element
  ...
  describe('form item accessibility integration', () => {
    it('automatic id attachment', async () => {
      ...
      const colorPickerButton = wrapper.find('.k-color-picker')
  ...
  it('it will target the focus & blur', async () => {
    ...
    await nextTick()
    await wrapper.find('.k-color-picker').trigger('focus')
    expect(focusHandler).toHaveBeenCalledTimes(1)

    await wrapper.find('.k-color-picker').trigger('blur')
