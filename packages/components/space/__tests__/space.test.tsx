...
describe('Space.vue', () => {
  it('render test', async () => {
    ...
    expect(wrapper.find('.k-space--vertical').exists()).toBe(true)
    expect(wrapper.find('.k-space').attributes('style')).toContain(
      'flex-wrap: wrap'
    )
  })
  ...
  it('sizes', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.k-space').attributes('style')).toContain('gap: 16px')
    ...
    await nextTick()
    expect(wrapper.find('.k-space').attributes('style')).toContain('gap: 30px')
    ...
    expect(wrapper.find('.k-space').attributes('style')).toContain('gap: 8px')
  })
  ...
  it('fill', async () => {
    ...
    expect(wrapper.find('.k-space__item').attributes('style')).toContain(
      'flex-grow: 1'
    )
    expect(wrapper.find('.k-space__item').attributes('style')).toContain(
      'min-width: 100%'
