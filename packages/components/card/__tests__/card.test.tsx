...
describe('Card.vue', () => {
  ...
  test('body style', () => {
    ...
    expect(wrapper.find('.k-card__body').attributes('style')).toBe(style)
  })

  test('body style with object', () => {
    const style = { 'font-size': '14px' }
    const wrapper = mount(() => <Card bodyStyle={style}>{AXIOM}</Card>)
    expect(wrapper.find('.k-card__body').attributes('style')).toBe(
      'font-size: 14px;'
    )
  })
  ...
  test('body style with array', () => {
    ...
    const wrapper = mount(() => <Card bodyStyle={style}>{AXIOM}</Card>)
    expect(
      wrapper.find('.k-card__body').attributes('style')?.replace(/[ ]/g, '')
  ...
  test('should render body-class props', async () => {
    ...
    expect(wrapper.find('.k-card__body').classes()).toContain(bodyClass)
  })
