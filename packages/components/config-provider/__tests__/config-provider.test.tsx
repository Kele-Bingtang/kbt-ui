...
import Chinese from '@kbt-ui/locale/lang/zh-cn'
import English from '@kbt-ui/locale/lang/en'
import { KButton, KMessage } from '@kbt-ui/components'
...
describe('config-provider', () => {
  ...
  describe('button-config', () => {
    it('autoInsertSpace', async () => {
      ...
      await nextTick()
      expect(
        wrapper.find('.k-button .k-button__text--expand').exists()
      ).toBeTruthy()
    ...
    it('reactive namespace', async () => {
      ...
      const wrapper = mount(() => (
        <ConfigProvider namespace={namespace.value}>
          <KButton>test str</KButton>
        </ConfigProvider>
      ))

      await nextTick()
      expect(wrapper.find('button').classes().join('')).toBe('k-button')
      namespace.value = 'ep'
      await nextTick()
  ...
  describe('message-config', () => {
    ...
    it('limit the number of messages displayed at the same time', async () => {
      ...
      const open = () => {
        KMessage('this is a message.')
      }

      const wrapper = mount(() => (
        <ConfigProvider message={config}>
          <KButton onClick={open}>open</KButton>
        </ConfigProvider>
      ))

      await nextTick()
      wrapper.find('.k-button').trigger('click')
      wrapper.find('.k-button').trigger('click')
      wrapper.find('.k-button').trigger('click')
      wrapper.find('.k-button').trigger('click')
      await nextTick()
      expect(document.querySelectorAll('.k-message').length).toBe(3)
      ...
      wrapper.find('.k-button').trigger('click')
      await nextTick()
      expect(document.querySelectorAll('.k-message').length).toBe(7)
    })
    ...
    it('multiple config-provider config override', async () => {
      ...
      await wrapper.find('.k-button').trigger('click')
      await nextTick()
      expect(document.querySelectorAll('.k-message').length).toBe(1)
    })
  })
