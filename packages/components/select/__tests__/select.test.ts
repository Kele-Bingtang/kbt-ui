...
import { hasClass } from '@kbt-ui/utils'
import { KFormItem } from '@kbt-ui/components/form'
...
function getOptions(): HTMLElement[] {
  return Array.from(
    document.querySelectorAll<HTMLElement>(
      'body > div:last-child .k-select-dropdown__item'
    )
  )
}
...
const CLASS_NAME = 'k-select'
const WRAPPER_CLASS_NAME = 'k-select__wrapper'
const OPTION_ITEM_CLASS_NAME = 'k-select-dropdown__item'
const PLACEHOLDER_CLASS_NAME = 'k-select__placeholder'
...
describe('Select', () => {
  ...
  test('create', async () => {
    wrapper = _mount(`<k-select v-model="value"></k-select>`, () => ({
      value: '',
    }))
    ...
    const select = wrapper.findComponent({ name: 'KSelect' })
  ...
  test('custom dropdown class', () => {
    ...
    const dropdown = wrapper.findComponent({ name: 'KSelectDropdown' })
  ...
  test('custom label with object', async () => {
    wrapper = _mount(
      `
      <k-select v-model="value" value-key="id">
        <k-option
          v-for="item in options"
          :label="item.name"
          :key="item.id"
  ...
  test('single select', async () => {
    wrapper = _mount(
      `
      <k-select v-model="value" @change="handleChange">
        <k-option
          v-for="item in options"
          :label="item.label"
          :key="item.value"
  ...
  test('disabled select', () => {
    wrapper = _mount(`<k-select disabled></k-select>`)
  ...
  test('visible event', async () => {
    wrapper = _mount(
      `
    <k-select v-model="value" @visible-change="handleVisibleChange">
      <k-option
        v-for="item in options"
        :label="item.label"
        :key="item.value"
  ...
  test('suffix icon', async () => {
    wrapper = _mount(`<k-select></k-select>`)
  ...
  test('fitInputWidth', async () => {
    ...
    const selectRef = wrapper.findComponent({ name: 'KSelect' })
  ...
  test('multiple select', async () => {
    ...
    const tagCloseIcons = wrapper.findAll('.k-tag__close')
  ...
  test('multiple select when content overflow', async () => {
    wrapper = _mount(
      `
      <k-select v-model="selectedList" multiple placeholder="请选择">
        <k-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
        </k-option>
      </k-select>
    `,
      () => ({
    ...
    options[2].click()
    await nextTick()
    const tagWrappers = wrapper.findAll('.k-tag')
  ...
  test('multiple select with collapseTags when content overflow', async () => {
    wrapper = _mount(
      `
      <k-select v-model="selectedList" multiple collapseTags placeholder="请选择">
        <k-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
        </k-option>
      </k-select>
    `,
  ...
  test('multiple select with collapseTagsTooltip', async () => {
    wrapper = _mount(
      `
      <k-select v-model="selectedList" multiple collapseTags collapse-tags-tooltip placeholder="请选择">
        <k-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
        </k-option>
      </k-select>
    `,
    ...
    options[2].click()
    await nextTick()
    const triggerWrappers = wrapper.findAll('.k-tooltip__trigger')
    expect(triggerWrappers[0]).toBeDefined()
    const tags = document.querySelectorAll('.k-select__tags-text')
  ...
  test('multiple select with maxCollapseTags', async () => {
    wrapper = _mount(
      `
      <k-select v-model="selectedList" multiple collapseTags :max-collapse-tags="3" placeholder="请选择">
        <k-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
        </k-option>
      </k-select>
    `,
  ...
  test('multiple remove-tag', async () => {
    wrapper = _mount(
      `
      <k-select v-model="value" multiple @remove-tag="handleRemoveTag">
        <k-option
          v-for="item in options"
          :label="item.label"
          :key="item.value"
  ...
  test('only emit change on user input', async () => {
    ...
    wrapper = _mount(
      `
    <k-select v-model="value" @change="change" ref="select">
      <k-option label="1" value="1" />
      <k-option label="2" value="2" />
      <k-option label="3" value="3" />
    </k-select>`,
      () => ({
        value: '1',
        change: () => ++callCount,
  ...
  test('should set placeholder to label of selected option when filterable is true and multiple is false', async () => {
    wrapper = _mount(
      `
      <k-select ref="select" v-model="value" filterable>
    ...
    const selectVm = wrapper.findComponent({ name: 'KSelect' }).vm as any
  ...
  test('emptyText error show', async () => {
    wrapper = _mount(
      `
    <k-select :model-value="value" filterable placeholder="Select">
      <k-option
        v-for="item in options"
        :key="item.value"
        :label="item.label"
    ...
    await nextTick()
    expect(
      !!(document.querySelector('.k-select__popper') as HTMLElement).style
        .display
    ).toBeFalsy()
    expect(wrapper.findAll('.k-select-dropdown__empty').length).toBe(0)
  })
  ...
  test('multiple select with remote load', async () => {
    ...
    wrapper = mount({
      ...
      components: { KSelect: Select, KOption: Option },
      data() {
        return {
          options: [],
          value: [],
    ...
    const select = wrapper.findComponent({ name: 'KSelect' }).vm
    select.onInput({
      target: {
        value: '',
      },
  ...
  test('k-option-group should visible when k-option in a component', async () => {
  ...
  test('tag of disabled option is not closable', async () => {
    wrapper = _mount(
      `
    <k-select v-model="vendors" multiple :collapse-tags="isCollapsed" :clearable="isClearable" placeholder="Select Business Unit">
    <k-option
    ...
    await nextTick()
    const selectVm = wrapper.findComponent({ name: 'KSelect' }).vm as any
    expect(wrapper.findAll('.k-tag').length).toBe(3)
    const tagCloseIcons = wrapper.findAll('.k-tag__close')
    expect(tagCloseIcons.length).toBe(1)
    await tagCloseIcons[0].trigger('click')
    expect(wrapper.findAll('.k-tag__close').length).toBe(0)
    expect(wrapper.findAll('.k-tag').length).toBe(2)
    ...
    await nextTick()
    expect(
      wrapper.findAll('.k-tag').filter((item) => {
        return !hasClass(item.element, 'in-tooltip')
      }).length
    ).toBe(2)
    await wrapper.find('.k-tag__close').trigger('click')
    expect(
      wrapper.findAll('.k-tag').filter((item) => {
        return !hasClass(item.element, 'in-tooltip')
      }).length
    ...
    await nextTick()
    expect(
      wrapper.findAll('.k-tag__close').filter((item) => {
  ...
  test('tag type', async () => {
    wrapper = _mount(
      `
      <k-select v-model="value" multiple tag-type="success">
        <k-option
          v-for="item in options"
          :key="item.value"
          :label="item.label"
    ...
    options[1].click()
    await nextTick()
    expect(wrapper.find('.k-tag').classes()).toContain('k-tag--success')
  })

  test('modelValue should be deep reactive in multiple mode', async () => {
    wrapper = _mount(
      `
    <k-select v-model="modelValue" multiple>
      <k-option
    ...
    const vm = wrapper.vm as any
    await nextTick()
    expect(wrapper.findAll('.k-tag').length).toBe(1)

    vm.modelValue.splice(0, 1)

    await nextTick()
    expect(wrapper.findAll('.k-tag').length).toBe(0)
  })
  ...
  test('should reset placeholder after clear when both multiple and filterable are true', async () => {
    ...
    wrapper = _mount(
      `
    <k-select v-model="modelValue" multiple filterable placeholder=${placeholder}>
      <k-option label="1" value="1" />
    </k-select>`,
      () => ({
        modelValue: ['1'],
    ...
    const tagCloseIcon = wrapper.find('.k-tag__close')
  ...
  describe('should show all options when open select dropdown', () => {
    async function testShowOptions({ filterable, multiple }: SelectProps = {}) {
      ...
      const options = wrapper.findAllComponents({ name: 'KOption' })
  ...
  describe('teleported API', () => {
    ...
    it('should not mount on the popper container', async () => {
      ...
      wrapper = _mount(
        `
      <k-select v-model="modelValue" multiple :teleported="false">
        <k-option
          v-for="option in options"
          :key="option.value"
          :value="option.value"
  ...
  describe('form item accessibility integration', () => {
    it('automatic id attachment', async () => {
      const wrapper = _mount(
        `<k-form-item label="Foobar" data-test-ref="item">
          <k-select v-model="modelValue">
            <k-option label="1" value="1" />
          </k-select>
        </k-form-item>`,
      ...
      const formItemLabel = formItem.find('.k-form-item__label')
    ...
    it('specified id attachment', async () => {
      const wrapper = _mount(
        `<k-form-item label="Foobar" data-test-ref="item">
          <k-select id="foobar" v-model="modelValue">
            <k-option label="1" value="1" />
          </k-select>
        </k-form-item>`,
        () => ({
    ...
    it('should update selected data when the options prop is changed and the select is focused', async () => {
      ...
      vm.options = options
      await nextTick()
      expect(wrapper.findAll('.k-tag')[0].text()).toBe('option 1')
    })

    // fix: https://github.com/kbt-ui/kbt-ui/issues/11991
    it('backspace key should delete selected tag but should not delete disabled options', async () => {
      ...
      await nextTick()
      const selectInput = wrapper.find('.k-select__input')
      expect(wrapper.findAll('.k-tag').length).toBe(2)
      // after deletion, an k-tag will be deleted
      ...
      await nextTick()
      // after deletion, an k-tag still exist
      expect(wrapper.findAll('.k-tag').length).toBe(1)
    })
  })
  ...
  it('It should generate accessible attributes', async () => {
    wrapper = _mount(
      `<k-select v-model="value">
        <k-option label="label" value="1" />
        <k-option label="disabled" value="2" disabled />
      </k-select>`,
      () => ({ value: '1' })
    )
    ...
    const dropdown = wrapper.findComponent({ name: 'KSelectDropdown' })
    const input = wrapper.find('input')
    const list = dropdown.find('.k-select-dropdown__list')
    const option = dropdown.find('.k-select-dropdown__item')
    const disabledOption = dropdown.find(
      '.k-select-dropdown__item:nth-child(2)'
    )
