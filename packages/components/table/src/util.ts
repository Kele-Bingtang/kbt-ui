...
import { hasOwn, isArray, isBoolean, isObject, throwError } from '@kbt-ui/utils'
import KTooltip, { type KTooltipProps } from '@kbt-ui/components/tooltip'
...
export const getColumnByKey = function <T>(
  ...
  if (!column)
    throwError('KTable', `No column matching with column-key: ${columnKey}`)
  return column
}
