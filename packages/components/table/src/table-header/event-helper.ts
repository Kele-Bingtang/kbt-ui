...
function useEvent<T>(props: TableHeaderProps<T>, emit) {
  ...
  const handleMouseDown = (event: MouseEvent, column: TableColumnCtx<T>) => {
    ...
    if (draggingColumn.value && props.border) {
      ...
      const tableK = table?.vnode.el
      const tableLeft = tableK.getBoundingClientRect().left
      const columnK = instance.vnode.el.querySelector(`th.${column.id}`)
      const columnRect = columnK.getBoundingClientRect()
      const minLeft = columnRect.left - tableLeft + 30

      addClass(columnK, 'noclick')
