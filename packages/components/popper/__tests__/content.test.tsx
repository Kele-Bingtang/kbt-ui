...
import KContent from '../src/content.vue'
...
const TestComponent = defineComponent({
  ...
  render() {
    return (
      <KContent ref="contentRef" {...this.$attrs}>
        {AXIOM}
      </KContent>
    )
  },
...
const mountContent = (props = {}) =>
  mount(<KContent {...props}>{AXIOM}</KContent>, {
    global: {
      provide: {
...
describe('<KPopperContent />', () => {
  describe('with triggerRef provided', () => {
    const triggerKls = 'k-popper__trigger'
    ...
    it('should mount the component correctly and set popperInstance correctly', async () => {
      ...
      expect(wrapper.classes()).toEqual(['k-popper', 'is-dark'])
    ...
    it('should be able to be pure and themed', async () => {
      ...
      expect(wrapper.classes()).toEqual(['k-popper', 'is-pure', 'is-custom'])
    })
