...
import { useFormItem, useFormSize } from '@kbt-ui/components/form'
import { ClickOutside as vClickoutside } from '@kbt-ui/directives'
...
import { CHANGE_EVENT, EVENT_CODE, UPDATE_MODEL_EVENT } from '@kbt-ui/constants'
import { ArrowDown, Check, CircleClose } from '@element-plus/icons-vue'
import { cascaderEmits, cascaderProps } from './cascader'

import type { Options } from '@kbt-ui/components/popper'
import type { ComputedRef, Ref, StyleValue } from 'vue'
import type { TooltipInstance } from '@kbt-ui/components/tooltip'
import type { InputInstance } from '@kbt-ui/components/input'
import type { ScrollbarInstance } from '@kbt-ui/components/scrollbar'
import type {
  CascaderNode,
  CascaderPanelInstance,
  CascaderValue,
  Tag,
} from '@kbt-ui/components/cascader-panel'
...
const COMPONENT_NAME = 'KCascader'
...
const updateStyle = () => {
  ...
  const tagWrapperK = tagWrapper.value
  const suggestionPanelK = suggestionPanel.value?.$el

  if (!isClient || !inputInner) return

  if (suggestionPanelK) {
    const suggestionList = suggestionPanelK.querySelector(
  ...
  if (tagWrapperK) {
    const { offsetHeight } = tagWrapperK
    const height =
      presentTags.value.length > 0
