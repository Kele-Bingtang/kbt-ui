...
import type KCascader from '@kbt-ui/components/cascader'
...
const TRIGGER = '.k-cascader'
const NODE = '.k-cascader-node'
const TAG = '.k-tag'
const SUGGESTION_ITEM = '.k-cascader__suggestion-item'
const SUGGESTION_PANEL = '.k-cascader__suggestion-panel'
const DROPDOWN = '.k-cascader__dropdown'
...
describe('Cascader.vue', () => {
  ...
  test('multiple mode', async () => {
    ...
    await firstTag.find('.k-tag__close').trigger('click')
  ...
  test('filterable in multiple mode', async () => {
    ...
    const input = wrapper.find('.k-cascader__search-input')
  ...
  test('placeholder disappear when resetForm', async () => {
    ...
    const wrapper = _mount(() => (
      <KForm model={model}>
        <KFormItem label="Activity name" prop="name">
          <Cascader
            v-model={model.name}
            options={OPTIONS}
            filterable
    ...
    wrapper.findComponent(KForm).vm.$.exposed!.resetFields()
    await nextTick()
  ...
  test('should be able to trigger togglePopperVisible outside the component', async () => {
    let cascader: InstanceType<typeof KCascader>
    const clickFn = () => {
  ...
  test('height should be changed by size when multiple', async () => {
    ...
    await nextTick()
    const inputK = wrapper.find('input').element as HTMLElement
    ...
    for (const size in sizeMap) {
      ...
      inputK.style.setProperty('--k-input-height', `${sizeMap[size]}px`)
      ...
      await nextTick()
      expect(inputK.style.height).toEqual(`${sizeMap[size] - 2}px`)
    }
  })
})
