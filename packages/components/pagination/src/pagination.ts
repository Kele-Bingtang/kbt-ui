...
const componentName = 'KPagination'
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const assertValidUsage = computed(() => {
      ...
      // <k-pagination ...otherProps :current-page="xxx" /> without corresponding listener is forbidden now
      ...
      if (props.layout.includes('sizes')) {
        if (!isAbsent(props.pageCount)) {
          ...
          if (!isAbsent(props.pageSize)) {
            ...
            // else page size is controlled by k-pagination internally
          }
        }
      }
      return true
