...
const assertCurrent = (wrapper: VueWrapper<any>, page: number) => {
  expect(wrapper.find('.k-pager li.is-active.number').text()).toBe(
    String(page)
  )
}
const assertPages = (wrapper: VueWrapper<any>, total: number) => {
  expect(wrapper.find('.k-pagination .k-pager li:last-child').text()).toBe(
    String(total)
  )
}
...
describe('Pagination', () => {
  describe('test invalid usages', () => {
    ...
    test('both absence of total & pageCount is invalid', async () => {
      ...
      expect(wrapper.find('.k-pagination').exists()).toBe(false)
      expect(console.warn).toHaveBeenCalled()
      total.value = 100
      await nextTick()
      expect(wrapper.find('.k-pagination').exists()).toBe(true)
    })
  ...
  describe('test layout & layout reactive change', () => {
    ...
    const layoutSelectorPairs = [
      ['sizes', '.k-pagination__sizes'],
      ['prev', 'button.btn-prev'],
      ['pager', 'ul.k-pager'],
      ['next', 'button.btn-next'],
      ['jumper', '.k-pagination__jump'],
      ['total', '.k-pagination__total'],
    ]
    ...
    test(`layout with '->, total'`, async () => {
      ...
      assertElementsExistence(
        wrapper,
        ['.k-pagination__total', '.k-pagination__rightwrapper'],
        true
      )
    })
    ...
    test('test small layout', () => {
      ...
      expect(wrapper.vm.$el.classList.contains('k-pagination--small')).toBe(
        true
      )
    })
  ...
  describe('test pageSize & currentPage reactive change', () => {
    ...
    test('test currentPage change', async () => {
      ...
      await wrapper.find('.k-pager li:last-child').trigger('click')
  ...
  describe('test a11y supports', () => {
    test('test a11y attributes', async () => {
      ...
      const prev = wrapper.find('.k-pagination .btn-prev')
      const next = wrapper.find('.k-pagination .btn-next')
      const pagers = wrapper.findAll('.k-pagination .k-pager .number')
    ...
    test('test tabindex interactive', async () => {
      const wrapper = mount(() => <Pagination total={100} />)
      await wrapper.find('.k-pager li:nth-child(2)').trigger('click')
      assertCurrent(wrapper, 2)
      await wrapper.find('.k-pager li:nth-child(3)').trigger('click', {
        key: 'Enter',
      })
      assertCurrent(wrapper, 3)
      // TODO getComputedStyle is not implemented in jsdom, so I duno how to assert style of psuedo-class
      /*
       * await wrapper.find('.k-pager li:nth-child(3)').trigger('keyup', {
       *   key: 'Tab',
       * })
       * const style = window.getComputedStyle(wrapper.find('.k-pager li:nth-child(4)').element, ':focus-visible')
    ...
    test('test tabindex disabled', async () => {
      ...
      expect(
        wrapper.find('.k-pager li:first-child').attributes('tabindex')
      ).toBe('-1')
