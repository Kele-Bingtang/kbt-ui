...
describe('virtual scrollbar', () => {
  ...
  it('click track', async () => {
    ...
    expect(wrapper.find('.k-scrollbar__thumb').attributes('style')).toContain(
      initializeStyle
    )
    ...
    expect(
      wrapper.find('.k-scrollbar__thumb').attributes('style')
  ...
  it('horizontal track height/width', async () => {
    ...
    expect(
      (wrapper.find('.k-virtual-scrollbar').element as HTMLElement).style.width
    ).toContain('198px') // clientSize - props.endGap = 200 - 2 = 198

    expect(
      (wrapper.find('.k-virtual-scrollbar').element as HTMLElement).style.height
