...
import triggerCompositeClick from '@kbt-ui/test-utils/composite-click'
...
describe('Dialog.vue', () => {
  test('render test', async () => {
    ...
    await rAF()
    await nextTick()
    expect(wrapper.find('.k-dialog__body').text()).toEqual(AXIOM)
  })
  ...
  test('dialog should have a title and header when it has been given', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.k-dialog__header').text()).toBe(HEADER)
  ...
  test('dialog should have a footer when footer has been given', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.k-dialog__footer').exists()).toBe(true)
    expect(wrapper.find('.k-dialog__footer').text()).toBe(AXIOM)
  })
  ...
  test('should append dialog to body when appendToBody is true', async () => {
    ...
    await nextTick()
    expect(
      document.body.firstElementChild!.classList.contains('k-overlay')
    ).toBe(true)
    wrapper.unmount()
  })
  ...
  test('should center dialog', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.k-dialog--center').exists()).toBe(true)
  })
  ...
  test('should show close button', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.k-dialog__close').exists()).toBe(true)
  })
  ...
  test('should hide close button when showClose = false', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.k-dialog__headerbtn').exists()).toBe(false)
  })
  ...
  test('should close dialog when click on close button', async () => {
    ...
    await nextTick()
    await wrapper.find('.k-dialog__headerbtn').trigger('click')
  ...
  describe('mask related', () => {
    test('should not have overlay mask when mask is false', async () => {
      ...
      await nextTick()
      expect(wrapper.find('.k-overlay').exists()).toBe(false)
    })
    ...
    test('should close the modal when clicking on mask when `closeOnClickModal` is true', async () => {
      ...
      await nextTick()
      expect(wrapper.find('.k-overlay').exists()).toBe(true)
      expect(wrapper.find('.k-overlay-dialog').exists()).toBe(true)

      await triggerCompositeClick(wrapper.find('.k-overlay-dialog'))
      expect(wrapper.vm.visible).toBe(false)
    })
  })
  ...
  describe('life cycles', () => {
    ...
    test('should destroy on close', async () => {
      ...
      await rAF()
      await nextTick()
      expect(wrapper.find('.k-dialog__body').exists()).toBe(false)
    })
  ...
  describe('accessibility', () => {
    ...
    test('missing title attribute should point to header slot content', async () => {
      ...
      await nextTick()
      const dialog = wrapper.find('[role="dialog"]')
      const dialogTitle = wrapper.find('.k-dialog__title')
    ...
    test('aria-describedby should point to modal body', async () => {
      ...
      await nextTick()
      const dialog = wrapper.find('[role="dialog"]')
      const dialogBody = wrapper.find('.k-dialog__body')
