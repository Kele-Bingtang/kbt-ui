...
import { IMAGE_SUCCESS } from '@kbt-ui/test-utils/mock'
...
describe('<image-viewer />', () => {
  test('big image preview', async () => {
    ...
    await doubleWait()
    const viewer = wrapper.find('.k-image-viewer__wrapper')
    expect(viewer.exists()).toBe(true)
    await wrapper.find('.k-image-viewer__close').trigger('click')
  ...
  test('image preview hide-click-on-modal', async () => {
    ...
    const viewer = wrapper.find('.k-image-viewer__wrapper')
    expect(viewer.exists()).toBe(true)
    await wrapper.find('.k-image-viewer__mask').trigger('click')
  ...
  test('manually switch image', async () => {
    ...
    const imgList = wrapper.findAll('.k-image-viewer__img')
