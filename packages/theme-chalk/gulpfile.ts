...
import { epOutput } from '@kbt-ui/build-utils'
...
function buildThemeChalk() {
  ...
  const noKPrefixFile = /(index|base|display)/
  return src(path.resolve(__dirname, 'src/*.scss'))
    ...
    .pipe(
      rename((path) => {
        if (!noKPrefixFile.test(path.basename)) {
          path.basename = `k-${path.basename}`
        }
      })
    )
...
/**
 * copy from packages/theme-chalk/dist to dist/kbt-ui/theme-chalk
 */
