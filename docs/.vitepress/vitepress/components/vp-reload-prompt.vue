...
<template>
  <transition name="pwa-popup">
    <k-card v-if="!alwaysRefresh && needRefresh" class="pwa-card" role="alert">
      <p class="pwa-card-text">{{ locale.message }}</p>
      <k-button type="primary" plain @click="updateServiceWorker()">
        {{ locale.refresh }}
      </k-button>
      <k-button plain @click="alwaysRefresh = true">
        {{ locale['always-refresh'] }}
      </k-button>
      <k-button plain @click="needRefresh = false">
        {{ locale.close }}
      </k-button>
    </k-card>
  </transition>
...
.pwa-popup-leave-active {
  transition: var(--k-transition-md-fade);
}
