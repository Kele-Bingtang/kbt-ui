...
:::demo Call `KMessageBox.confirm` method to open a confirm, and it simulates the system's `confirm`. We can also highly customize Message Box by passing a third attribute `options` which is a literal object. The attribute `type` indicates the message type, and it's value can be `success`, `error`, `info` and `warning`. Note that the second attribute `title` must be a `string`, and if it is an `object`, it will be handled as the attribute `options`. Here we use `Promise` to handle further processing.
...
If Kbt UI is fully imported, it will add the following global methods for `app.config.globalProperties`: `$msgbox`, `$alert`, `$confirm` and `$prompt`. So in a Vue instance you can call `MessageBox` like what we did in this page. The parameters are:
...
## App context inheritance <k-tag>> 2.0.4</k-tag>
...
import { KMessageBox } from 'kbt-ui'

// in your setup method
const { appContext } = getCurrentInstance()!
// You can pass it like:
KMessageBox({}, appContext)
// or if you are using variants
KMessageBox.alert('Hello world!', 'Title', {}, appContext)
```
...
The corresponding methods are: `KMessageBox`, `KMessageBox.alert`, `KMessageBox.confirm` and `KMessageBox.prompt`. The parameters are the same as above.
