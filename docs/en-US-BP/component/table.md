...
:::demo Use `row-class-name` in `k-table` to add custom classes to a certain row. Then you can style it with custom classes.
...
:::demo By setting the attribute `height` of `k-table`, you can fix the table header without any other codes.
...
:::demo Attribute `fixed` is used in `k-table-column`, it accepts a `Boolean`. If `true`, the column will be fixed at left. It also accepts two string literals: 'left' and 'right', both indicating that the column will be fixed at corresponding direction.
...
:::demo By setting the attribute `max-height` of `k-table`, you can fix the table header. The table body scrolls only if the height of the rows exceeds the max height value.
...
:::demo Only need to place k-table-column inside a k-table-column, you can achieve group header.
...
:::demo The attribute `fixed` of the group header is determined by the outermost `k-table-column`
...
:::demo Set attribute `filters` and `filter-method` in `k-table-column` makes this column filterable. `filters` is an array, and `filter-method` is a function deciding which rows are displayed. It has three parameters: `value`, `row` and `column`.
...
:::demo Activate expandable row by adding type="expand" and slot. The template for k-table-column will be rendered as the contents of the expanded row, and you can access the same attributes as when you are using `slot` in custom column templates.
...
:::demo To customize row indices, use `index` attribute on `k-table-column` with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
...
The [table-layout](https://developer.mozilla.org/zh-CN/docs/Web/CSS/table-layout) property sets the algorithm used to lay out table cells, rows, and columns.
...
| tooltip-options ^(2.2.28) | the options for the overflow tooltip, [see the following tooltip component](tooltip.html#attributes)                                                                                                                                                                       | ^[object]`Pick<KTooltipProps, 'effect' \| 'enterable' \| 'hideAfter' \| 'offset' \| 'placement' \| 'popperClass' \| 'popperOptions' \| 'showAfter' \| 'showArrow'>` | ^[object]`{ enterable: true, placement: 'top', showArrow: true, hideAfter: 200, popperOptions: { strategy: 'fixed' } }` |
...
| resizable                  | whether column width can be resized, works when `border` of `k-table` is `true`                                                                                                                                    | ^[boolean]                                                                                                                                                                  | true                              |
...
```vue
<k-table-column label="Thumbnail" width="180">
    <template #default="scope">
        <div style="display: flex; align-items: center">
            <k-image :preview-src-list="srcList"/>
        </div>
    </template>
</k-table-column>
```
...
Typical issue: [#5046](https://github.com/kbt-ui/kbt-ui/issues/5046) [#5862](https://github.com/kbt-ui/kbt-ui/issues/5862) [#6919](https://github.com/kbt-ui/kbt-ui/issues/6919)
