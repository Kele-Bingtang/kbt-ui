...
:::demo Combine `k-radio-group` with `k-radio` to display a radio group. Bind a variable with `v-model` of `k-radio-group` element and set label value in `k-radio`. It also provides `change` event with the current value as its parameter.
...
:::demo You just need to change `k-radio` element into `k-radio-button` element. We also provide `size` attribute.
