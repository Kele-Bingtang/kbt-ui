...
Text button has been upgraded with a new design since <k-tag round effect="plain" size="small">2.2.0</k-tag> , if you want to use the
...
:::demo Use the `icon` attribute to add icon. You can find the icon list in Kbt UI icon component. Adding icons to the right side of the text is achievable with an `<i>` tag. Custom icons can be used as well.
...
:::demo Use tag `<k-button-group>` to group your buttons.
