...
After version `2.5.0`, the default width of `k-select` changed to `100%`. When used in a inline form, the width will collapse. In order to display the width properly, you need to give `k-select` a specific width.
...
:::demo `v-model` is the value of `k-option` that is currently selected.
...
:::demo Set the value of `disabled` in `k-option` to `true` to disable this option.
...
:::demo Set `disabled` of `k-select` to make it disabled.
...
:::demo Set `clearable` attribute for `k-select` and a clear icon will appear. Note that `clearable` is only for single select.
...
:::demo Set `multiple` attribute for `k-select` to enable multiple mode. In this case, the value of `v-model` will be an array of selected options. By default the selected options will be displayed as Tags. You can collapse them to a text by using `collapse-tags` attribute. You can check them when mouse hover collapse text by using `collapse-tags-tooltip` attribute.
...
:::demo Insert customized HTML templates into the slot of `k-option`.
...
:::demo Use `k-option-group` to group the options, and its `label` attribute stands for the name of the group.
...
:::demo Adding `filterable` to `k-select` enables filtering. By default, Select will find all the options whose `label` attribute contains the input value. If you prefer other filtering strategies, you can pass the `filter-method`. `filter-method` is a `Function` that gets called when the input value changes, and its parameter is the current input value.
...
:::demo Set the value of `filterable` and `remote` with `true` to enable remote search, and you should pass the `remote-method`. `remote-method` is a `Function` that gets called when the input value changes, and its parameter is the current input value. Note that if `k-option` is rendered with the `v-for` directive, you should add the `key` attribute for `k-option`. Its value needs to be unique, such as `item.value` in the following example.
