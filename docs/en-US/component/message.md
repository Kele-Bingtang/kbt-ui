...
:::demo The setup of Message is very similar to notification, so parts of the options won't be explained in detail here. You can check the options table below combined with notification doc to understand it. Kbt UI has registered a `$message` method for invoking. Message can take a string or a VNode as parameter, and it will be shown as the main body.
...
Kbt UI has added a global method `$message` for `app.config.globalProperties`. So in a vue instance you can call `Message` like what we did in this page.
...
In this case you should call `KMessage(options)`. We have also registered methods for different types, e.g. `KMessage.success(options)`. You can call `KMessage.closeAll()` to manually close all the instances.
...
If you globally registered KMessage component, it will automatically inherit your app context.
