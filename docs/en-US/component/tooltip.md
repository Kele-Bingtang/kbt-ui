...
:::demo Override attribute `content` of `k-tooltip` by adding a slot named `content`.
...
`transition` attribute allows you to customize the animation in which the tooltip shows or hides, and the default value is k-fade-in-linear.

`disabled` attribute allows you to disable `tooltip`. You just need set it to `true`.

In fact, Tooltip is an extension based on [KPopper](https://github.com/kbt-ui/kbt-ui/tree/dev/packages/components/popper), you can use any attribute that are allowed in KPopper.
...
Disabled form elements are not supported for Tooltip, more information can be found at [MDN](https://developer.mozilla.org/zh-CN/docs/Web/Events/mouseenter). You need to wrap the disabled form element with a container element for Tooltip to work.
...
| -------------------- | ---------------------------------------------------------------- | ------------------------------------------------- |
| popperRef            | k-popper component instance                                      | ^[object]`Ref<PopperInstance \| null>`            |
| contentRef           | k-tooltip-content component instance                             | ^[object]`Ref<TooltipContentInstance \| null>`    |
| isFocusInsideContent | validate current focus event is trigger inside k-tooltip-content | ^[Function]`() => boolean \| undefined`           |
| updatePopper         | update k-popper component instance                               | ^[Function]`() => void`                           |
| onOpen               | expose onOpen function to mange k-tooltip open state             | ^[Function]`(event?: Event \| undefined) => void` |
| onClose              | expose onClose function to mange k-tooltip open state            | ^[Function]`(event?: Event \| undefined) => void` |
